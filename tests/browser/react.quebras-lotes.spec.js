 ...
 * formulários se desenham, e que as moradas de sempre continuam em
 * Livewire. Mexer no stock a sério prova-se na API.
 */
...
test('as quebras abrem com o periodo e o relatorio', async ({ page }) => {
    await page.goto('/invoicing/quebras/novo-ecra');
...
test('os lotes abrem com os cartoes e o modal do lote novo', async ({ page }) => {
    await page.goto('/invoicing/product-batches/novo-ecra');
...
test('as moradas de sempre continuam em Livewire', async ({ page }) => {
    for (const m of ['/invoicing/quebras', '/invoicing/product-batches']) {
        await page.goto(m);
