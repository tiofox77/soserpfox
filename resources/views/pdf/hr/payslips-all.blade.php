...
<head>
    ...
    <title>Recibos - {{ $payroll->payroll_number }} ({{ $payrollItems->count() }} funcionarios)</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ── Page ── */
        .employee-page { max-width: 210mm; margin: 0 auto 20px; background: white; box-shadow: 0 4px 24px rgba(0,0,0,.12); border-radius: 4px; padding: 8mm 10mm; }
        .employee-page:first-of-type { margin-top: 76px; }
        ...
        @media print {
            ...
            .employee-page { margin: 0; box-shadow: none; border-radius: 0; padding: 0; max-width: none; page-break-after: always; }
            .employee-page:last-child { page-break-after: avoid; }
            .employee-page:first-of-type { margin-top: 0; }
...
<body>
    ...
    <div class="toolbar">
        <div>
            <div class="toolbar-title">Recibos de Pagamento</div>
            <div class="toolbar-sub">{{ $payroll->payroll_number }} - {{ $payrollItems->count() }} funcionarios</div>
        </div>
        <div class="toolbar-actions">
            <button class="btn btn-back" onclick="window.close(); return false;">&#10005; Fechar</button>
            <button class="btn btn-print" onclick="window.print();">&#128424; Imprimir Todos</button>
        </div>
    </div>
    ...
    @foreach(['empresa', 'funcionario'] as $via)
        ...
        <div class="via">
            ...
            <div class="doc-title">
                ...
                <span>Periodo: {{ $payroll->period_start->format('d/m/Y') }} a {{ $payroll->period_end->format('d/m/Y') }} | Ref: {{ $payroll->payroll_number }}</span>
            </div>
