@php
    // ============================================
    // TEMPLATE PAGINADO GENÉRICO PARA DOCUMENTOS
    // ============================================
    
    // Detectar tipo de documento
    $docType = $documentType ?? 'invoice';
    $docNumber = $document->invoice_number ?? $document->proforma_number ?? $document->credit_note_number ?? $document->debit_note_number ?? $document->receipt_number ?? $document->advance_number ?? 'N/A';
    ...
    $docTitle = $docTitles[$docType] ?? 'DOCUMENTO';
    ...
    $primaryColor = $docColors[$docType] ?? '#2c5aa0';
    
    // Cliente
    $clientName = $document->client->name ?? 'N/A';
    $clientNif = $document->client->nif ?? '999999999';
    $clientAddress = $document->client->address ?? '';
    $clientCity = $document->client->city ?? '';
    $clientPhone = $document->client->phone ?? $document->client->mobile ?? '';
    $clientEmail = $document->client->email ?? '';
    
    // Datas
    $docDate = $document->invoice_date ?? $document->proforma_date ?? $document->issue_date ?? $document->receipt_date ?? $document->created_at;
...
<body>
    ...
    <div class="page">
            ...
            <div class="header-content">
                <div class="company-section">
                    <div class="logo">
                        ...
                        @else
                            <div style="width: 90px; height: 55px; background: {{ $primaryColor }}; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 10px;">LOGO</div>
                        @endif
                    </div>
                ...
                <div class="client-section">
                    ...
                    <div class="client-nif">NIF: {{ $clientNif }}</div>
                    <div class="company-details">
                        @if($clientAddress){{ $clientAddress }}<br>@endif
                        @if($clientCity){{ $clientCity }}<br>@endif
                        @if($clientPhone)Tel: {{ $clientPhone }}@endif
        ...
        <div class="doc-header">
            <div class="doc-title">{{ $docTitle }} N.º {{ $docNumber }}</div>
        ...
        <table class="info-table">
            ...
            <tr>
                <td>{{ $document->currency ?? 'AOA' }}</td>
                <td>{{ $docDate ? \Carbon\Carbon::parse($docDate)->format('d/m/Y') : 'N/A' }}</td>
                <td>{{ $document->created_at ? $document->created_at->format('H:i') : 'N/A' }}</td>
                <td>{{ $dueDate ? \Carbon\Carbon::parse($dueDate)->format('d/m/Y') : 'N/A' }}</td>
                <td>{{ $document->creator->name ?? 'Sistema' }}</td>
                    ...
                    <div class="footer-left">
                        ...
                        <table class="tax-table">
                            ...
                            <tr>
                                <td>IVA</td>
                                <td>14%</td>
                                <td class="currency">{{ number_format($document->subtotal ?? 0, 2, ',', '.') }}</td>
                                <td class="currency">{{ number_format($document->tax_amount ?? 0, 2, ',', '.') }}</td>
                            </tr>
                        </table>
                        ...
                        <div class="system-info">
                            ...
                            @if($document->saft_hash)
                                | HASH: "{{ substr($document->saft_hash, -4) }}"
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="footer-right">
                        <div class="totals-box">
                            <div class="totals-row">
                                ...
                                <span>{{ number_format($document->subtotal ?? 0, 2, ',', '.') }} Kz</span>
                            </div>
                            <div class="totals-row">
                                <span>Desc. Comercial</span>
                                <span>{{ number_format($document->discount_commercial ?? 0, 2, ',', '.') }} Kz</span>
                            </div>
                            <div class="totals-row">
                                <span>Desc. Financeiro</span>
                                <span>{{ number_format($document->discount_financial ?? 0, 2, ',', '.') }} Kz</span>
                            </div>
                            <div class="totals-row">
                                <span>IVA (14%)</span>
                                <span>{{ number_format($document->tax_amount ?? 0, 2, ',', '.') }} Kz</span>
                            </div>
                            <div class="totals-row">
                                <span>Retenção</span>
                                <span>{{ number_format($document->irt_amount ?? 0, 2, ',', '.') }} Kz</span>
                            </div>
