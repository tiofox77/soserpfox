    ...
    <div class="page-wrapper">
        <div class="main-content">
            ...
            @endif
                ...
                <div class="right-section">
                    <div class="supplier-info">
                        ...
                        <div class="supplier-name">{{ $receipt->client->name }}</div>
                        <div class="supplier-nif">NIF: {{ $receipt->client->nif ?? 'N/D' }}</div>
            ...
            <div class="doc-header">
                <div class="doc-title" style="text-align: left;">Recibo n.º {{ $receipt->receipt_number }}</div>
                @if($receipt->invoice)
                <div style="font-size: 9px; color: #666; margin-top: 5px; text-align: left;">
                    Referente à Fatura: {{ $receipt->invoice->invoice_number }} ({{ $receipt->invoice->invoice_date->format('d/m/Y') }})
                </div>
                @endif
                <div style="font-size: 9px; color: #666; margin-top: 5px; text-align: left;">
                    <strong>Tipo:</strong> {{ $receipt->type === 'sale' ? 'Recibo de Venda' : 'Recibo de Compra' }}
                </div>
            </div>
            ...
            {{-- Box de Valor do Recibo --}}
            <div style="background: linear-gradient(135deg, #e0f2fe 0%, #f0f9ff 100%); border: 3px solid #0066cc; border-radius: 10px; padding: 30px; margin: 30px 0; text-align: center;">
                <div style="font-size: 11px; font-weight: bold; color: #0066cc; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1px;">
                    💰 VALOR RECEBIDO
                </div>
                <div style="font-size: 36px; font-weight: bold; color: #0066cc; margin: 15px 0;">
                ...
                <div style="margin-top: 20px; padding: 15px; background-color: white; border-radius: 5px; text-align: left; font-size: 9px; border-left: 4px solid #0066cc;">
                    <strong style="color: #0066cc;">📝 Observações:</strong><br>
                    <span style="color: #333; margin-top: 5px; display: block;">{{ $receipt->notes }}</span>
                </div>
                @endif
            </div>
                ...
                <div class="right-bottom">
                    <div class="summary-section">
                        <div style="text-align: center; padding: 15px 0; border-bottom: 1px solid #e5e7eb;">
                            <div style="font-size: 9px; color: #666; margin-bottom: 8px;">RESUMO DO RECIBO</div>
                        </div>
                        
                        @if($receipt->invoice)
                        <div class="summary-row">
                            <span>Fatura Ref.</span>
                            <span>{{ $receipt->invoice->invoice_number }}</span>
                        </div>
                        @endif
                        ...
                        <div style="margin: 15px 0; border-top: 2px solid #0066cc; padding-top: 15px;">
                            <div class="summary-row summary-total" style="font-size: 11px;">
                                <span style="font-weight: bold;">VALOR PAGO</span>
                                <span style="font-weight: bold;">{{ number_format($receipt->amount, 2, ',', '.') }}</span>
                            </div>
                        </div>
