<div class="p-6">
    @include("livewire.invoicing.partials.cabecalho-criar-documento", [
        "titulo"    => $isEdit ? "Editar Fatura de Compra" : "Nova Fatura de Compra",
        "subtitulo" => "Documento recebido de fornecedor",
        "icone"     => "fa-file-import",
        "cor"       => "emerald",
        "voltar"    => route("invoicing.purchases.invoices"),
    ])
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="mt-2 max-h-60 overflow-y-auto border-2 border-gray-200 rounded-xl bg-white shadow-lg">
                                ...
                                <div wire:click="selectSupplier({{ $client->id }})"
                                     class="p-3 hover:bg-orange-50 cursor-pointer transition border-b border-gray-100 last:border-b-0">
                            ...
                            @if($supplier_id && !$searchSupplier)
                                @php
                                    $selectedClient = $suppliers->where('id', $supplier_id)->first();
                                @endphp
                                @if($selectedClient)
                                <div class="mt-2 p-3 {{ $selectedClient->nif === '999999999' ? 'bg-yellow-50 border-2 border-yellow-300' : 'bg-orange-50 border-2 border-orange-200' }} rounded-xl">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-orange-50 transition" wire:key="cart-item-{{ $item->id }}">
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-75 z-50 flex items-center justify-center p-4 animate-fade-in">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
            ...
            <form wire:submit.prevent="createQuickSupplier">
                <div class="p-6 space-y-4">
                    {{-- Name --}}
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">
        ...
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('openInvoicePreview', (event) => {
                ...
                if (invoiceId) {
                    ...
                    const previewUrl = '{{ url("/invoicing/purchases/invoices") }}/' + invoiceId + '/preview';
                    ...
                    <div wire:click="addProduct({{ $product->id }})"
                        ...
                        <div class="flex flex-col h-full">
                            ...
                            <div class="mt-auto pt-3 border-t border-gray-200">
                                <p class="text-lg font-bold text-orange-600">{{ number_format($product->sale_price, 2) }} Kz</p>
                                <p class="text-xs text-gray-500 mt-1">
