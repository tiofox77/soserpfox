{{-- History Modal --}}
@if($showHistoryModal && \$invoiceHistory)
<div class="fixed inset-0 bg-gray-900 bg-opacity-75 z-50 flex items-center justify-center p-4" wire:click.self="closeHistoryModal">
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        {{-- Header --}}
        <div class="bg-gradient-to-r from-orange-600 to-red-600 px-6 py-4 rounded-t-2xl flex items-center justify-between">
            <div>
                <h3 class="text-xl font-bold text-white flex items-center">
                    <i class="fas fa-history mr-2"></i>
                    Histórico de Conversões
                </h3>
                <p class="text-orange-100 text-sm mt-1">Fatura: {{ \$invoiceHistory->invoice_number }}</p>
            </div>
        ...
        <div class="p-6">
            ...
            <div class="bg-orange-50 rounded-xl p-4 mb-6">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <p class="font-semibold text-gray-900">{{ \$invoiceHistory->supplier->name }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Data</p>
                        <p class="font-semibold text-gray-900">{{ \$invoiceHistory->invoice_date->format('d/m/Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Total</p>
                        <p class="font-semibold text-orange-600">{{ number_format(\$invoiceHistory->total, 2) }} Kz</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Status</p>
                        <span class="inline-flex px-3 py-1 text-xs font-bold rounded-full
                            {{ \$invoiceHistory->status === 'draft' ? 'bg-gray-100 text-gray-800' : '' }}
                            {{ \$invoiceHistory->status === 'sent' ? 'bg-blue-100 text-blue-800' : '' }}
                            {{ \$invoiceHistory->status === 'accepted' ? 'bg-green-100 text-green-800' : '' }}">
                            {{ \$invoiceHistory->status === 'draft' ? 'Rascunho' : '' }}
                            {{ \$invoiceHistory->status === 'sent' ? 'Enviada' : '' }}
                            {{ \$invoiceHistory->status === 'accepted' ? 'Aceite' : '' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <h4 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
                    ...
                    Faturas Geradas ({{ count(\$relatedDocuments) }})
                </h4>
                
                @if(count(\$relatedDocuments) > 0)
                    <div class="space-y-3">
                        @foreach(\$relatedDocuments as $invoice)
        ...
        <div class="bg-gray-50 px-6 py-4 rounded-b-2xl flex justify-between items-center">
            <button wire:click="convertToInvoice({{ \$invoiceHistory->id }})" 
