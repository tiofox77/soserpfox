{{-- History Modal --}}
@if($showHistoryModal && $quoteHistory)
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-4 rounded-t-2xl flex items-center justify-between">
            <div>
                ...
                <p class="text-purple-100 text-sm mt-1">{{ __('Orçamento: :numero', ['numero' => $quoteHistory->quote_number]) }}</p>
            </div>
        ...
        <div class="p-6">
            ...
            <div class="bg-purple-50 rounded-xl p-4 mb-6">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <p class="text-xs text-gray-500 mb-1">{{ __('Cliente') }}</p>
                        <p class="font-semibold text-gray-900">{{ $quoteHistory->client->name }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">{{ __('Data') }}</p>
                        <p class="font-semibold text-gray-900">{{ $quoteHistory->quote_date->format('d/m/Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">{{ __('Total') }}</p>
                        <p class="font-semibold text-purple-600">{{ number_format($quoteHistory->total, 2) }} Kz</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">{{ __('Status') }}</p>
                        <span class="inline-flex px-3 py-1 text-xs font-bold rounded-full
                            {{ $quoteHistory->status === 'draft' ? 'bg-gray-100 text-gray-800' : '' }}
                            {{ $quoteHistory->status === 'sent' ? 'bg-blue-100 text-blue-800' : '' }}
                            {{ $quoteHistory->status === 'accepted' ? 'bg-green-100 text-green-800' : '' }}">
                            {{ $quoteHistory->status === 'draft' ? __('Rascunho') : '' }}
                            {{ $quoteHistory->status === 'sent' ? __('Enviado') : '' }}
                            {{ $quoteHistory->status === 'accepted' ? __('Aceite') : '' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <h4 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
                    ...
                    {{ __('Faturas Geradas') }} ({{ count($relatedInvoices) }})
                </h4>
                            ...
                            <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">
                                                @if($invoice->creator)
                                                    {{ __('Criada em :data por :autor', ['data' => $invoice->created_at->format('d/m/Y H:i'), 'autor' => $invoice->creator->name]) }}
                                                @else
                                                    {{ __('Criada em :data', ['data' => $invoice->created_at->format('d/m/Y H:i')]) }}
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div class="ml-4 flex flex-col gap-2">
                                    <span class="inline-flex px-3 py-1 text-xs font-bold rounded-full
                                        ...
                                        {{ $invoice->status === 'draft' ? __('Rascunho') : '' }}
                                        {{ $invoice->status === 'pending' ? __('Pendente') : '' }}
                                        {{ $invoice->status === 'paid' ? __('Pago') : '' }}
                                        {{ $invoice->status === 'cancelled' ? __('Cancelado') : '' }}
                                    </span>
        ...
        <div class="bg-gray-50 px-6 py-4 rounded-b-2xl flex justify-between items-center">
            <button wire:click="convertToInvoice({{ $quoteHistory->id }})"
                    class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl font-semibold transition">
                <i class="fas fa-plus mr-2"></i>{{ __('Converter Novamente') }}
            </button>
