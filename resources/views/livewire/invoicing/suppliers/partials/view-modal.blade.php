        ...
        <div class="relative w-full bg-white rounded-2xl text-left shadow-2xl transform transition-all my-4 sm:my-8 sm:max-w-7xl max-h-[94vh] overflow-y-auto">
            ...
            <div class="bg-gray-50 border-b border-gray-200 px-2 sm:px-6">
                <nav class="flex flex-wrap gap-1 -mb-px overflow-x-auto" aria-label="Tabs">
                    <button @click="tab = 'info'" :class="tab === 'info' ? 'border-orange-600 text-orange-700 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                        ...
                        <i class="fas fa-truck mr-1.5"></i>{{ __('Info Fornecedor') }}
                    </button>
                    ...
                    <button @click="tab = 'produtos'" :class="tab === 'produtos' ? 'border-orange-600 text-orange-700 bg-white' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                            class="whitespace-nowrap py-3 px-4 border-b-2 font-semibold text-sm transition">
                        <i class="fas fa-box mr-1.5"></i>{{ __('Produtos') }}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-white rounded-xl shadow p-5">
                            <h4 class="text-xs font-bold text-gray-500 uppercase mb-3 flex items-center">
                                <i class="fas fa-truck mr-2 text-orange-500"></i>{{ __('Identificação') }}
                            </h4>
                            <dl class="space-y-2 text-sm">
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Nome:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['name'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('NIF:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['nif'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Tipo:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ ($viewingSupplier['type'] ?? '') === 'pessoa_juridica' ? 'Pessoa Jurídica' : 'Pessoa Física' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('ID:') }}</dt><dd class="font-mono text-gray-700 text-right">{{ $viewingSupplier['id'] ?? '—' }}</dd></div>
                            </dl>
                        </div>
                        ...
                        <div class="bg-white rounded-xl shadow p-5">
                            ...
                            <dl class="space-y-2 text-sm">
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Email:') }}</dt><dd class="font-semibold text-gray-900 text-right break-all">{{ $viewingSupplier['email'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Telefone:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['phone'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Celular:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['mobile'] ?? '—' }}</dd></div>
                            </dl>
                        </div>
                        ...
                        <div class="bg-white rounded-xl shadow p-5 md:col-span-2">
                            ...
                            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('País:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['country'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Província:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['province'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Cidade:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['city'] ?? '—' }}</dd></div>
                                <div class="flex justify-between"><dt class="text-gray-500">{{ __('Cód. Postal:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['postal_code'] ?? '—' }}</dd></div>
                                <div class="sm:col-span-2 flex justify-between"><dt class="text-gray-500">{{ __('Endereço:') }}</dt><dd class="font-semibold text-gray-900 text-right">{{ $viewingSupplier['address'] ?? '—' }}</dd></div>
                            </dl>
                        </div>
                ...
                <div x-show="tab === 'detalhes'" x-transition>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-blue-500">
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ __('Compras registadas') }}</p>
                        </div>

                        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-orange-500">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-orange-600 uppercase">{{ __('Comprado') }}</span>
                        ...
                        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-emerald-500">
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ __('Total pago') }}</p>
                        </div>
                        ...
                        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-amber-500">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-amber-600 uppercase">{{ __('Frequência') }}</span>
                                <i class="fas fa-calendar-alt text-amber-500"></i>
                            </div>
                            <p class="text-2xl font-bold text-gray-900">{{ $supplierStats['avg_days_between'] ?? 0 }}<span class="text-sm text-gray-500"> {{ __('dias') }}</span></p>
                        ...
                        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-cyan-500">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-cyan-600 uppercase">{{ __('Pagamentos') }}</span>
                        ...
                        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-pink-500">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold text-pink-600 uppercase">{{ __('Produtos') }}</span>
                                <i class="fas fa-box text-pink-500"></i>
                            </div>
                            <p class="text-2xl font-bold text-gray-900">{{ count($supplierTopProducts) }}</p>
                            <p class="text-xs text-gray-500 mt-1">{{ __('Distintos comprados') }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-xl shadow overflow-hidden">
                        <div class="px-5 py-3 bg-gradient-to-r from-blue-50 to-cyan-50 border-b border-gray-200 flex items-center justify-between">
                            <h4 class="font-bold text-gray-800 flex items-center text-sm">
                                <i class="fas fa-list-alt mr-2 text-blue-600"></i>{{ __('Últimas 20 Faturas de Compra') }}
                            </h4>
                ...
                <div x-show="tab === 'produtos'" x-transition>
                    <div class="bg-white rounded-xl shadow overflow-hidden">
                        <div class="px-5 py-3 bg-gradient-to-r from-purple-50 to-indigo-50 border-b border-gray-200">
                            <h4 class="font-bold text-gray-800 flex items-center text-sm">
                                <i class="fas fa-box mr-2 text-purple-600"></i>{{ __('Top 10 Produtos Comprados a Este Fornecedor') }}
                            </h4>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-xs sm:text-sm">
            ...
            <div class="bg-gray-100 px-6 py-3 flex items-center justify-end gap-2 border-t border-gray-200">
                <button wire:click="edit({{ $viewingSupplier['id'] ?? 0 }})" class="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-semibold transition">
                    <i class="fas fa-edit mr-1"></i>{{ __('Editar Fornecedor') }}
                </button>
