<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 stagger-animation">
        <!-- Total Fornecedores -->
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-orange-100 overflow-hidden card-hover card-3d">
            <div class="flex items-center justify-between mb-4">
                <div class="w-14 h-14 bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl flex items-center justify-center shadow-lg shadow-orange-500/50 icon-float">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @if($dateFrom)
                        <span class="inline-flex items-center px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-semibold">
                            ...
                            <button wire:click="$set('dateFrom', '')" class="ml-2 hover:text-green-900">
                                <i class="fas fa-times"></i>
                            </button>
                        </span>
                    @endif
            ...
            @forelse($suppliers as $supplier)
                <div class="group grid grid-cols-12 gap-4 px-6 py-4 hover:bg-orange-50 transition-all duration-300 items-center">
                    ...
                    <div class="col-span-3 flex items-center space-x-3">
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-white font-bold shadow-lg flex-shrink-0">
                            {{ strtoupper(substr($supplier->name, 0, 2)) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="font-bold text-gray-900 truncate">{{ $supplier->name }}</p>
                    ...
                    <div class="col-span-2">
                        ...
                        @if($supplier->phone)
                            <p class="text-xs text-gray-500 flex items-center">
                                <i class="fas fa-phone text-green-500 mr-1.5"></i>{{ $supplier->phone }}
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        @if($supplier->country)
                            <p class="text-xs text-gray-500 flex items-center">
                                <i class="fas fa-globe text-blue-500 mr-1.5"></i>{{ $supplier->country }}
                            </p>
                        @endif
                    </div>
                    ...
                    <!-- Tipo -->
                    <div class="col-span-2">
                        <span class="inline-flex items-center px-2.5 py-1 {{ $supplier->type === 'pessoa_juridica' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }} rounded-lg text-xs font-semibold">
                            <i class="fas {{ $supplier->type === 'pessoa_juridica' ? 'fa-building' : 'fa-user' }} mr-1"></i>
                            {{ $supplier->type === 'pessoa_juridica' ? 'Empresa' : 'Pessoa Física' }}
                        </span>
                    </div>
