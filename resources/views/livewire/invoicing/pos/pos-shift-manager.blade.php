<div>
    ...
    <div class="mb-6 bg-gradient-to-r from-purple-600 to-indigo-600 rounded-2xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <div>
                    <h2 class="text-2xl font-bold">{{ __('POS - Ponto de Venda') }}</h2>
    ...
    @if(!$currentShift)
        ...
        <div class="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <h3 class="text-2xl font-bold text-gray-900 mb-2">{{ __('Nenhum Turno Aberto') }}</h3>
            <p class="text-gray-600 mb-6">{{ __('Abra um turno para começar a vender no POS') }}</p>
            <button wire:click="openShiftModal" class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-8 py-4 rounded-xl font-bold hover:shadow-xl transition">
                <i class="fas fa-play mr-2"></i>{{ __('Abrir Novo Turno') }}
            </button>
        </div>
    @else
        {{-- Turno aberto --}}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
                    ...
                    {{ __('Informações do Turno') }}
                </h3>
                <div class="space-y-3">
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span class="text-sm text-gray-600">{{ __('Número') }}</span>
                        <span class="font-bold text-purple-600">{{ $currentShift->shift_number }}</span>
                    </div>
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span class="text-sm text-gray-600">{{ __('Status') }}</span>
                        {{-- O status_label vem do modelo PosShift (Aberto/Fechado) e ainda
                             fala português: traduzi-lo aqui com __($variavel) daria uma
                             chave que nenhum varrimento estático apanha. Fica para o lote
                             que embrulhar os modelos. --}}
                    ...
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span class="text-sm text-gray-600">{{ __('Aberto em') }}</span>
                        <span class="font-bold text-gray-900">{{ $currentShift->opened_at->format('H:i') }}</span>
                    </div>
                    <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span class="text-sm text-gray-600">{{ __('Duração') }}</span>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-3">
                    <div class="p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl">
                        <p class="text-xs text-blue-600 font-semibold mb-1">{{ __('Saldo Inicial') }}</p>
                    ...
                    <div class="p-4 bg-gradient-to-r from-green-50 to-green-100 rounded-xl">
                        <p class="text-xs text-green-600 font-semibold mb-1">{{ __('Total de Vendas') }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-3">
                    <div class="p-4 bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl">
                        <p class="text-xs text-purple-600 font-semibold mb-1">{{ __('Total de Faturas') }}</p>
                    ...
                    <div class="p-4 bg-gradient-to-r from-orange-50 to-orange-100 rounded-xl">
                        <p class="text-xs text-orange-600 font-semibold mb-1">{{ __('Total de Recibos') }}</p>
                        <p class="text-3xl font-bold text-orange-900">{{ $currentShift->total_receipts }}</p>
                    </div>
                    <div class="p-4 bg-gray-50 rounded-lg">
                        <p class="text-xs text-gray-500 mb-1">{{ __('Valor Médio') }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b-2 border-gray-200">
                                <th class="text-left p-3 text-sm font-semibold text-gray-700">{{ __('Tipo') }}</th>
                                <th class="text-left p-3 text-sm font-semibold text-gray-700">{{ __('Referência') }}</th>
                                <th class="text-left p-3 text-sm font-semibold text-gray-700">{{ __('Método') }}</th>
                                <th class="text-right p-3 text-sm font-semibold text-gray-700">{{ __('Valor') }}</th>
                                <th class="text-right p-3 text-sm font-semibold text-gray-700">{{ __('Data') }}</th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-500">{{ __('Nenhuma transação registrada ainda') }}</p>
                </div>
            @endif
        </div>
    @endif
    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            ...
            <div class="p-6 space-y-4">
                <div>
                    {{-- "Kz" fora do __(): é moeda, não muda de língua nenhuma, e
                         deixá-lo dentro obrigava a repetir o rótulo por moeda. --}}
                    <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('Saldo Inicial') }} (Kz) *</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('Observações (opcional)') }}</label>
                    <textarea wire:model="opening_notes" rows="3"
                              ...
                              placeholder="{{ __('Notas sobre a abertura do turno...') }}"></textarea>
                </div>
                ...
                <div class="flex space-x-3 pt-4 border-t">
                    <button wire:click="openShift"
                        ...
                        <span wire:loading.remove>
                            <i class="fas fa-check mr-2"></i>{{ __('Abrir Turno') }}
                        </span>
                        <span wire:loading>
    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('Dinheiro Contado') }} (Kz) *</label>
                    ...
                    @if($actual_cash > 0 && $difference != 0)
                        ...
                        @php
                            $__valorDiferenca = ($difference > 0 ? '+' : '')
                                . number_format($difference, 2, ',', '.') . ' Kz';
                        @endphp
                        <div class="mt-2 p-3 rounded-lg {{ $difference > 0 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200' }}">
                            <p class="text-sm font-bold {{ $difference > 0 ? 'text-green-900' : 'text-red-900' }}">
                                {{ $difference > 0
                                    ? __('Diferença: :valor (Sobra)', ['valor' => $__valorDiferenca])
                                    : __('Diferença: :valor (Falta)', ['valor' => $__valorDiferenca]) }}
                            </p>
                        </div>
                    @endif
                </div>
                ...
                @if($actual_cash > 0 && abs($difference) > 0.01)
                    <div>
                        ...
                        <textarea wire:model="difference_reason" rows="2"
                                  ...
                                  placeholder="{{ __('Explique o motivo da diferença...') }}"></textarea>
                    </div>
                @endif

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('Observações de Fechamento (opcional)') }}</label>
                    <textarea wire:model="closing_notes" rows="3"
                              class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
                              placeholder="{{ __('Notas sobre o fechamento...') }}"></textarea>
                </div>
                ...
                <div class="flex space-x-3 pt-4 border-t">
                    <button wire:click="closeShift"
                        ...
                        <span wire:loading.remove>
                            <i class="fas fa-lock mr-2"></i>{{ __('Fechar Turno') }}
                        </span>
                        <span wire:loading>
                            <i class="fas fa-spinner fa-spin mr-2"></i>{{ __('Fechando...') }}
                        </span>
                    </button>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-4 flex items-center justify-between">
                <h3 class="text-xl font-bold text-white">
                    <i class="fas fa-check-circle mr-2"></i>{{ __('Turno Fechado!') }}
                </h3>
            ...
            <div class="p-6 space-y-4">
                <p class="text-gray-700 text-center">
                    {{ __('Deseja imprimir o resumo do turno?') }}
                </p>
                ...
                <div class="grid grid-cols-1 gap-3">
                    ...
                    <a href="{{ route('invoicing.pos.export.shift-ticket', $lastClosedShiftId) }}"
                       ...
                       class="flex items-center justify-center gap-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-6 py-4 rounded-xl font-semibold shadow-md transition">
                        ...
                        <span>{{ __('Imprimir Ticket Térmico (80mm)') }}</span>
                    </a>
                    ...
                    <a href="{{ route('invoicing.pos.export.shift-pdf', $lastClosedShiftId) }}"
                       ...
                       class="flex items-center justify-center gap-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white px-6 py-4 rounded-xl font-semibold shadow-md transition">
                        ...
                        <span>{{ __('Descarregar PDF A4 Resumo') }}</span>
                    </a>
                    ...
                    <button wire:click="closeAfterCloseModal"
                        ...
                        <span>{{ __('Fechar sem Imprimir') }}</span>
                    </button>
                </div>
            </div>
        </div>
