<div>
    <div class="mb-6 bg-gradient-to-r from-orange-600 to-red-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">{{ __('Mapa de Compras') }}</h2><p class="text-orange-100 text-sm">{{ __('Faturas de compra por período') }}</p></div>
        </div>
    ...
    <x-report-filters color="orange">
        <x-slot:extra>
            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">{{ __('Fornecedor') }}</label>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-orange-500"><p class="text-xs font-bold text-orange-600 uppercase">{{ __('Total') }}</p><p class="text-lg font-bold mt-1 text-orange-700">{{ number_format($totals['total'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-red-500"><p class="text-xs font-bold text-red-600 uppercase">{{ __('A Pagar') }}</p><p class="text-lg font-bold mt-1 text-red-700">{{ number_format($totals['pending'], 2, ',', '.') }}</p></div>
    </div>

    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-5 py-3 bg-gray-50 border-b flex items-center justify-between">
            <h3 class="font-bold text-gray-800"><i class="fas fa-list mr-2 text-orange-600"></i>{{ __('Faturas no Período') }}</h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
                        <th class="px-3 py-2 text-left">{{ __('Nº') }}</th><th class="px-3 py-2 text-left">{{ __('Data') }}</th>
                        <th class="px-3 py-2 text-left">{{ __('Fornecedor') }}</th>
                        <th class="px-3 py-2 text-right">{{ __('Subtotal') }}</th><th class="px-3 py-2 text-right">IVA</th>
                        <th class="px-3 py-2 text-right">{{ __('Total') }}</th><th class="px-3 py-2 text-right">{{ __('Pago') }}</th>
                ...
                <tbody class="divide-y">
                    ...
                    @empty
                        <tr><td colspan="8" class="px-3 py-8 text-center text-gray-400 italic">{{ __('Sem compras no período') }}</td></tr>
                    @endforelse
                </tbody>
                @if($invoices->count())
