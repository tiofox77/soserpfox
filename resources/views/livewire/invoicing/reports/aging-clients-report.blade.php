<div>
    <div class="mb-6 bg-gradient-to-r from-indigo-600 to-blue-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">{{ __('Aging de Clientes') }}</h2><p class="text-indigo-100 text-sm">{{ __('Antiguidade de saldos por faixa de dias') }}</p></div>
        </div>
        <div class="flex gap-2">
            <button onclick="window.print()" class="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg text-sm font-semibold transition"><i class="fas fa-print mr-1"></i>{{ __('Imprimir') }}</button>
    ...
    <div class="grid grid-cols-2 md:grid-cols-6 gap-2 mb-6">
        <div class="bg-white rounded-xl shadow p-3 border-l-4 border-green-500"><p class="text-xs font-bold text-green-600 uppercase">{{ __('A Vencer') }}</p><p class="text-sm font-bold mt-1">{{ number_format($bucketTotals['current'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-3 border-l-4 border-yellow-500"><p class="text-xs font-bold text-yellow-600 uppercase">{{ __('1-30 dias') }}</p><p class="text-sm font-bold mt-1">{{ number_format($bucketTotals['days_30'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-3 border-l-4 border-orange-500"><p class="text-xs font-bold text-orange-600 uppercase">{{ __('31-60 dias') }}</p><p class="text-sm font-bold mt-1">{{ number_format($bucketTotals['days_60'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-3 border-l-4 border-red-500"><p class="text-xs font-bold text-red-600 uppercase">{{ __('61-90 dias') }}</p><p class="text-sm font-bold mt-1">{{ number_format($bucketTotals['days_90'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-3 border-l-4 border-red-700"><p class="text-xs font-bold text-red-700 uppercase">{{ __('91-120 dias') }}</p><p class="text-sm font-bold mt-1">{{ number_format($bucketTotals['days_120'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-3 border-l-4 border-purple-700"><p class="text-xs font-bold text-purple-700 uppercase">{{ __('+120 dias') }}</p><p class="text-sm font-bold mt-1">{{ number_format($bucketTotals['over_120'], 2, ',', '.') }}</p></div>
    </div>

    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-5 py-3 bg-gray-50 border-b">
            <h3 class="font-bold text-gray-800"><i class="fas fa-table mr-2 text-indigo-600"></i>{{ __('Saldos por Cliente — Total:') }} <span class="text-indigo-700">{{ number_format($bucketTotals['grand'], 2, ',', '.') }} Kz</span></h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
                        <th class="px-3 py-2 text-left">{{ __('Cliente') }}</th>
                        <th class="px-3 py-2 text-right text-green-700">{{ __('A Vencer') }}</th>
                ...
                <tbody class="divide-y">
                    ...
                    @empty
                        <tr><td colspan="8" class="px-3 py-8 text-center text-gray-400 italic">{{ __('Nenhum saldo em aberto') }}</td></tr>
                    @endforelse
                </tbody>
                @if(count($grouped))
                <tfoot class="bg-gray-100 font-bold">
                    <tr>
                        <td class="px-3 py-2 text-right">{{ __('TOTAIS:') }}</td>
