<div>
    <div class="mb-6 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">{{ __('Lucros e Perdas (DRE)') }}</h2><p class="text-emerald-100 text-sm">{{ __('Demonstração de resultados do período') }}</p></div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-xl shadow p-5 border-l-4 border-blue-500">
            <p class="text-xs font-bold text-blue-600 uppercase">{{ __('Receita Líquida') }}</p>
        ...
        <div class="bg-white rounded-xl shadow p-5 border-l-4 border-orange-500">
            <p class="text-xs font-bold text-orange-600 uppercase">{{ __('Custo (CMV)') }}</p>
            <p class="text-3xl font-bold mt-2 text-orange-700">{{ number_format($cogs, 2, ',', '.') }}</p>
            <p class="text-xs text-gray-500 mt-1">{{ __('Custo dos produtos vendidos') }}</p>
        </div>
        <div class="bg-white rounded-xl shadow p-5 border-l-4 {{ $netResult >= 0 ? 'border-emerald-500' : 'border-red-500' }}">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ __('Margem:') }} <strong>{{ number_format($grossMargin, 1, ',', '.') }}%</strong></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
        <div class="px-5 py-3 bg-gradient-to-r from-emerald-50 to-teal-50 border-b">
            <h3 class="font-bold text-gray-800"><i class="fas fa-table mr-2 text-emerald-600"></i>{{ __('Demonstração de Resultados') }}</h3>
        </div>
        <table class="w-full text-sm">
            <tbody class="divide-y">
                <tr><td class="px-5 py-3 font-semibold">{{ __('(+) Receita Bruta de Vendas') }}</td><td class="px-5 py-3 text-right font-bold">{{ number_format($grossRevenue, 2, ',', '.') }}</td></tr>
                <tr class="text-red-600"><td class="px-5 py-3 pl-10">{{ __('(−) Descontos concedidos') }}</td><td class="px-5 py-3 text-right">({{ number_format($discounts, 2, ',', '.') }})</td></tr>
                <tr class="text-red-600"><td class="px-5 py-3 pl-10">{{ __('(−) Devoluções (Notas de Crédito)') }}</td><td class="px-5 py-3 text-right">({{ number_format($returns, 2, ',', '.') }})</td></tr>
                <tr class="bg-blue-50 font-bold"><td class="px-5 py-3">{{ __('(=) Receita Líquida') }}</td><td class="px-5 py-3 text-right text-blue-700">{{ number_format($netRevenue, 2, ',', '.') }}</td></tr>
                <tr class="text-orange-600"><td class="px-5 py-3 pl-10">{{ __('(−) Custo dos Produtos Vendidos (CMV)') }}</td><td class="px-5 py-3 text-right">({{ number_format($cogs, 2, ',', '.') }})</td></tr>
                ...
                <tr class="bg-gray-50"><td class="px-5 py-3 text-xs text-gray-500 italic">{{ __('Despesas operacionais não modeladas (incluir manualmente na contabilidade)') }}</td><td class="px-5 py-3 text-right text-xs text-gray-500">Referência compras: {{ number_format($expenses, 2, ',', '.') }}</td></tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-5 py-3 bg-gray-50 border-b">
            <h3 class="font-bold text-gray-800"><i class="fas fa-chart-area mr-2 text-emerald-600"></i>{{ __('Evolução dos Últimos 6 Meses') }}</h3>
        </div>
        <div class="p-5">
                ...
                <div class="mb-4">
                    <div class="flex items-center justify-between text-xs mb-1">
                        ...
                        <span class="flex-1 text-right text-gray-500">
                            {{ __('Rec:') }} <strong class="text-blue-700">{{ number_format($m['revenue'], 0, ',', '.') }}</strong>
                            {{ __('· CMV:') }} <strong class="text-orange-700">{{ number_format($m['cogs'], 0, ',', '.') }}</strong>
                            {{ __('· Lucro:') }} <strong class="{{ $m['profit'] >= 0 ? 'text-emerald-700' : 'text-red-700' }}">{{ number_format($m['profit'], 0, ',', '.') }}</strong>
                        </span>
                    </div>
                    <div class="space-y-1">
