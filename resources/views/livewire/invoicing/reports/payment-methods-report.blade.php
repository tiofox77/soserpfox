<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="mb-6 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold">{{ __('Recebimentos por Meio de Pagamento') }}</h1>
                    <p class="text-emerald-100 text-sm">{{ __('Total recebido por forma de pagamento no período') }}</p>
                </div>
            </div>
    ...
    <div class="mb-6 bg-white rounded-2xl shadow p-5">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">{{ __('Meio') }}</label>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div class="bg-white rounded-2xl shadow p-5 border border-emerald-100">
            <p class="text-xs text-gray-500 uppercase font-bold">{{ __('Total Recebido') }}</p>
        ...
        <div class="bg-white rounded-2xl shadow p-5 border border-teal-100">
            <p class="text-xs text-gray-500 uppercase font-bold">{{ __('Nº de Recebimentos') }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
        <div class="px-6 py-4 border-b bg-gray-50"><h3 class="font-bold text-gray-900"><i class="fas fa-chart-pie mr-2 text-emerald-600"></i>{{ __('Resumo por Meio de Pagamento') }}</h3></div>
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">{{ __('Meio') }}</th>
                    <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 uppercase">{{ __('Nº') }}</th>
                    <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 uppercase">{{ __('Total') }}</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-10 text-center text-gray-400"><i class="fas fa-inbox text-3xl mb-2"></i><p class="text-sm">{{ __('Sem recebimentos no período') }}</p></td></tr>
                @endforelse
            </tbody>
            @if($byMethod->count() > 0)
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">{{ __('Meio') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 uppercase">{{ __('Valor') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    ...
                    @empty
                        <tr><td colspan="4" class="px-6 py-10 text-center text-gray-400">{{ __('Sem recebimentos') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
