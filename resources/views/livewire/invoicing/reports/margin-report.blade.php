<div>
    <div class="mb-6 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">{{ __('Análise de Margem por Produto') }}</h2><p class="text-cyan-100 text-sm">{{ __('Lucro/prejuízo por produto vendido') }}</p></div>
        </div>
    ...
    <x-report-filters color="cyan">
        <x-slot:extra>
            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">{{ __('Ordenar por') }}</label>
            <select wire:model.live="sortBy" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                ...
                <option value="profit_asc">{{ __('Maior prejuízo') }}</option>
                <option value="margin_desc">{{ __('Maior margem %') }}</option>
                <option value="margin_asc">{{ __('Menor margem %') }}</option>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-blue-500"><p class="text-xs font-bold text-blue-600 uppercase">{{ __('Receita') }}</p><p class="text-lg font-bold mt-1">{{ number_format($totals['revenue'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-orange-500"><p class="text-xs font-bold text-orange-600 uppercase">{{ __('Custo') }}</p><p class="text-lg font-bold mt-1">{{ number_format($totals['cost'], 2, ',', '.') }}</p></div>
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-purple-500">
            <p class="text-xs font-bold text-purple-600 uppercase">{{ __('Margem Média') }}</p>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-right">{{ __('Custo Unit.') }}</th>
                        <th class="px-3 py-2 text-right">{{ __('Receita') }}</th>
                        <th class="px-3 py-2 text-right">{{ __('Custo Total') }}</th>
                ...
                <tbody class="divide-y">
                    ...
                    @empty
                        <tr><td colspan="9" class="px-3 py-8 text-center text-gray-400 italic">{{ __('Sem vendas no período') }}</td></tr>
                    @endforelse
                </tbody>
                @if($rows->count())
