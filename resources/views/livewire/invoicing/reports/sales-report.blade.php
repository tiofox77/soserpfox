<div>
    <div class="mb-6 bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div>
                <h2 class="text-2xl font-bold">Mapa de Vendas</h2>
                <p class="text-green-100 text-sm">Análise de faturas de venda por período</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-green-500"><p class="text-xs font-bold text-green-600 uppercase">Total</p><p class="text-lg font-bold mt-1 text-green-700">{{ number_format($totals['total'], 2, ',', '.') }}</p></div>
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-red-500"><p class="text-xs font-bold text-red-600 uppercase">Pendente</p><p class="text-lg font-bold mt-1 text-red-700">{{ number_format($totals['pending'], 2, ',', '.') }}</p></div>
    </div>

    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-5 py-3 bg-gray-50 border-b flex items-center justify-between">
            <h3 class="font-bold text-gray-800"><i class="fas fa-list mr-2 text-green-600"></i>Faturas no Período</h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y">
                    ...
                    @empty
                        <tr><td colspan="8" class="px-3 py-8 text-center text-gray-400 italic">Sem faturas no período</td></tr>
                    @endforelse
                </tbody>
                @if($invoices->count())
