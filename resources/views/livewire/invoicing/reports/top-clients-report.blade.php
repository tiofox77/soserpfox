<div>
    <div class="mb-6 bg-gradient-to-r from-yellow-500 to-amber-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mr-3"><i class="fas fa-crown text-2xl"></i></div>
            <div><h2 class="text-2xl font-bold">{{ __('Top Clientes') }}</h2><p class="text-yellow-100 text-sm">{{ __('Ranking de clientes por faturação') }}</p></div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-5 py-3 bg-gradient-to-r from-yellow-50 to-amber-50 border-b">
            <h3 class="font-bold text-gray-800"><i class="fas fa-trophy mr-2 text-amber-600"></i>{{ __('Ranking — Total faturado:') }} <span class="text-amber-700">{{ number_format($grandTotal, 2, ',', '.') }} Kz</span></h3>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-right">{{ __('Faturado') }}</th>
                ...
                <tbody class="divide-y">
                    @forelse($rows as $idx => $r)
                        @php $pct = $grandTotal > 0 ? ($r->total_revenue / $grandTotal * 100) : 0; @endphp
                        <tr class="hover:bg-amber-50">
                            ...
                            <td class="px-3 py-2 font-semibold">{{ $r->client->name ?? 'Cliente removido' }}</td>
                            <td class="px-3 py-2 text-right">{{ $r->invoices_count }}</td>
                            <td class="px-3 py-2 text-right font-bold text-amber-700">{{ number_format($r->total_revenue, 2, ',', '.') }}</td>
                            ...
                            <td class="px-3 py-2 w-1/4">
                                <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                    <div class="h-full bg-gradient-to-r from-yellow-400 to-amber-500" style="width: {{ $pct }}%"></div>
                                </div>
                            </td>
                        </tr>
                    @empty
