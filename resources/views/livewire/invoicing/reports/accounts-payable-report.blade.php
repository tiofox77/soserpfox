<div>
    <div class="mb-6 bg-gradient-to-r from-red-600 to-pink-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">Contas a Pagar</h2><p class="text-red-100 text-sm">Faturas de compra pendentes</p></div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-md p-5 mb-6 border-l-4 border-red-500 flex items-center justify-between">
        <div class="flex items-center gap-3">
            <h3 class="font-bold text-gray-800"><i class="fas fa-filter mr-2 text-red-600"></i>Filtro</h3>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-red-500"><p class="text-xs font-bold text-red-600 uppercase">Documentos</p><p class="text-2xl font-bold mt-1">{{ $invoices->count() }}</p></div>
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-gray-500"><p class="text-xs font-bold text-gray-600 uppercase">Total</p><p class="text-lg font-bold mt-1">{{ number_format($totals['total'], 2, ',', '.') }}</p></div>
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-orange-500"><p class="text-xs font-bold text-orange-600 uppercase">Saldo a Pagar</p><p class="text-lg font-bold mt-1 text-orange-700">{{ number_format($totals['balance'], 2, ',', '.') }}</p></div>
    </div>
    ...
    <div class="mb-4 bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg flex items-center">
        ...
        <div>
            <p class="font-bold text-orange-900">Existem faturas com pagamento vencido</p>
            <p class="text-sm text-orange-700">Total vencido: <strong>{{ number_format($overdueTotal, 2, ',', '.') }} Kz</strong></p>
        </div>
    </div>
    @endif
