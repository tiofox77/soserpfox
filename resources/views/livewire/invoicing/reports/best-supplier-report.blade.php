<div>
    <div class="mb-6 bg-gradient-to-r from-amber-600 to-yellow-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">Melhor Fornecedor</h2><p class="text-amber-100 text-sm">Score multi-critério (volume, frequência, fiabilidade, prazo)</p></div>
        </div>
            ...
            @foreach($stats->take(3) as $idx => $s)
                ...
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-{{ ['yellow', 'gray', 'orange'][$idx] }}-300">
                    <div class="bg-gradient-to-br {{ $medals[$idx] }} p-4 text-white text-center">
                        ...
                        <p class="text-xs opacity-90">Score (máx. 100)</p>
                    </div>
                    <div class="p-4">
                        <h3 class="font-bold text-gray-900 truncate">{{ $s->name }}</h3>
                        <p class="text-xs text-gray-500 mb-3">NIF: {{ $s->nif ?? '—' }}</p>
                        <div class="space-y-1 text-xs">
                            <div class="flex justify-between"><span class="text-gray-500">Volume comprado:</span><strong>{{ number_format($s->total_value, 0, ',', '.') }} Kz</strong></div>
                            <div class="flex justify-between"><span class="text-gray-500">Faturas:</span><strong>{{ $s->invoices_count }}</strong></div>
                            <div class="flex justify-between"><span class="text-gray-500">Fiabilidade:</span><strong class="text-emerald-700">{{ $s->reliability_pct }}%</strong></div>
                            <div class="flex justify-between"><span class="text-gray-500">Prazo médio:</span><strong>{{ number_format($s->avg_payment_term ?? 0, 0) }} dias</strong></div>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg mb-4 text-xs text-blue-900">
        <p class="font-bold mb-1"><i class="fas fa-info-circle mr-1"></i>Critérios do Score (máximo 100 pontos):</p>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
            <div><strong>Volume (30 pts):</strong> valor total comprado</div>
            <div><strong>Frequência (20 pts):</strong> nº de faturas no período</div>
            <div><strong>Fiabilidade (25 pts):</strong> ausência de faturas vencidas</div>
            <div><strong>Prazo (25 pts):</strong> prazo médio de pagamento concedido</div>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-right">Fiabilidade</th>
                        <th class="px-3 py-2 text-right">Prazo Médio</th>
                ...
                <tbody class="divide-y">
                    @forelse($stats as $idx => $s)
                        <tr class="hover:bg-amber-50">
                            ...
                            <td class="px-3 py-2 w-1/5">
                                <div class="flex h-3 rounded-full overflow-hidden bg-gray-100" title="Volume / Frequência / Fiabilidade / Prazo">
                    ...
                    @empty
                        <tr><td colspan="9" class="px-3 py-8 text-center text-gray-400 italic">Sem dados no período</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
