<div>
    <div class="mb-6 bg-gradient-to-r from-red-600 to-orange-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">{{ __('Mapa de IVA') }}</h2><p class="text-red-100 text-sm">{{ __('IVA liquidado, dedutível e a entregar') }}</p></div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-green-500">
            <p class="text-xs font-bold text-green-600 uppercase">{{ __('IVA Liquidado (Vendas)') }}</p>
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-orange-500">
            <p class="text-xs font-bold text-orange-600 uppercase">{{ __('IVA Dedutível (Compras)') }}</p>
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 {{ $totals['tax_payable'] >= 0 ? 'border-red-500' : 'border-emerald-500' }}">
            ...
            <p class="text-xs text-gray-500 mt-1">{{ __('Liquidado − Dedutível') }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            <div class="px-5 py-3 bg-green-50 border-b">
                <h3 class="font-bold text-green-800"><i class="fas fa-arrow-up mr-2"></i>{{ __('IVA Liquidado por Taxa') }}</h3>
            </div>
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
                        <th class="px-3 py-2 text-left">{{ __('Taxa') }}</th>
                ...
                <tbody class="divide-y">
                    ...
                    @empty
                        <tr><td colspan="3" class="px-3 py-6 text-center text-gray-400 italic">{{ __('Sem dados') }}</td></tr>
                    @endforelse
                </tbody>
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            <div class="px-5 py-3 bg-orange-50 border-b">
                <h3 class="font-bold text-orange-800"><i class="fas fa-arrow-down mr-2"></i>{{ __('IVA Dedutível por Taxa') }}</h3>
            </div>
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs uppercase text-gray-600">
                    <tr>
