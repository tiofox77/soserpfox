<div>
    <div class="mb-6 bg-gradient-to-r from-purple-600 to-fuchsia-600 rounded-2xl shadow-lg p-5 text-white flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div><h2 class="text-2xl font-bold">Mapa de Serviços</h2><p class="text-purple-100 text-sm">Análise específica de serviços prestados</p></div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-purple-500">
            <p class="text-xs font-bold text-purple-600 uppercase">Serviços Activos</p>
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-blue-500">
            <p class="text-xs font-bold text-blue-600 uppercase">Receita de Serviços</p>
        ...
        <div class="bg-white rounded-xl shadow p-4 border-l-4 {{ $totals['profit'] >= 0 ? 'border-emerald-500' : 'border-red-500' }}">
            <p class="text-xs font-bold {{ $totals['profit'] >= 0 ? 'text-emerald-600' : 'text-red-600' }} uppercase">Lucro</p>
            <p class="text-lg font-bold mt-1 {{ $totals['profit'] >= 0 ? 'text-emerald-700' : 'text-red-700' }}">{{ number_format($totals['profit'], 2, ',', '.') }}</p>
            <p class="text-xs text-gray-500 mt-1">Margem: <strong>{{ number_format($totals['margin'], 1, ',', '.') }}%</strong></p>
        </div>
        <div class="bg-white rounded-xl shadow p-4 border-l-4 border-cyan-500">
            <p class="text-xs font-bold text-cyan-600 uppercase">Alcance</p>
            <p class="text-2xl font-bold mt-1 text-cyan-700">{{ $totals['invoices'] }}</p>
            <p class="text-xs text-gray-500 mt-1">faturas com serviços</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow p-5 mb-6">
        <div class="flex items-center justify-between mb-2">
            <h3 class="font-bold text-gray-800"><i class="fas fa-chart-pie mr-2 text-purple-600"></i>Peso dos Serviços vs. Total de Vendas</h3>
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-5 py-3 bg-gradient-to-r from-purple-50 to-fuchsia-50 border-b">
            <h3 class="font-bold text-gray-800"><i class="fas fa-list mr-2 text-purple-600"></i>Detalhe por Serviço</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y">
                    ...
                    @empty
                        <tr><td colspan="9" class="px-3 py-8 text-center text-gray-400 italic">Nenhum serviço vendido no período</td></tr>
                    @endforelse
                </tbody>
                @if($rows->count())
