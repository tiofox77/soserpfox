<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="mb-6 bg-gradient-to-r from-orange-600 to-amber-600 rounded-2xl shadow-lg p-6 text-white">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold">Guias de Transporte / Remessa</h1>
                    <p class="text-orange-100 text-sm">Documentos de transporte de mercadorias (GT / GR)</p>
                </div>
            </div>
            <button wire:click="create" class="px-4 py-2 bg-white text-orange-600 rounded-lg hover:bg-orange-50 font-semibold text-sm">
                <i class="fas fa-plus mr-1"></i>Nova Guia
            </button>
        </div>
    </div>
    ...
    <div class="mb-4">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Pesquisar por nº..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Viatura</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 text-center whitespace-nowrap">
                                ...
                                @if(!in_array($g->agt_status, ['submitted','validated','accepted']))
                                    <button wire:click="submitAgt({{ $g->id }})" wire:loading.attr="disabled" class="text-emerald-600 hover:text-emerald-800 mr-3" title="Comunicar à AGT"><i class="fas fa-paper-plane"></i></button>
                                @endif
                                <button wire:click="confirmDelete({{ $g->id }})" class="text-red-500 hover:text-red-700" title="Anular"><i class="fas fa-ban"></i></button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="7" class="px-6 py-12 text-center text-gray-400"><i class="fas fa-truck text-4xl mb-2"></i><p class="text-sm">Nenhuma guia registada</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" style="backdrop-filter: blur(2px);">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
            <div class="sticky top-0 bg-white px-6 py-4 border-b flex items-center justify-between z-10">
                <h3 class="text-lg font-bold text-gray-900"><i class="fas fa-truck mr-2 text-orange-600"></i>Nova Guia</h3>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Fatura de origem (opcional)</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Cliente *</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Data de emissão *</label>
                        <input wire:model="issue_date" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Data/hora de carga</label>
                        <input wire:model="loading_datetime" type="datetime-local" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Matrícula da viatura</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Motorista</label>
                        <input wire:model="driver_name" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Doc. motorista</label>
                        <input wire:model="driver_document" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Local de carga</label>
                        <input wire:model="load_address" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Local de descarga</label>
                ...
                <div class="border-t pt-4">
                    <div class="flex items-end gap-2 mb-3">
                        <div class="flex-1">
                            <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Adicionar produto</label>
                    ...
                    <div class="border rounded-lg overflow-hidden">
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50"><tr>
                                <th class="px-3 py-2 text-left text-xs font-bold text-gray-500 uppercase">Descrição</th>
                                <th class="px-3 py-2 text-right text-xs font-bold text-gray-500 uppercase w-28">Qtd.</th>
                                <th class="px-3 py-2 text-left text-xs font-bold text-gray-500 uppercase w-20">Un.</th>
                                <th class="w-10"></th>
                            </tr></thead>
                            <tbody class="divide-y">
                                ...
                                @empty
                                    <tr><td colspan="4" class="px-3 py-6 text-center text-gray-400 text-sm">Sem itens. Escolha uma fatura de origem ou adicione produtos.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-600 mb-1 uppercase">Observações</label>
            ...
            <div class="sticky bottom-0 bg-white px-6 py-4 border-t flex justify-end gap-2">
                ...
                <button wire:click="save" wire:loading.attr="disabled" class="px-4 py-2 bg-orange-600 text-white rounded-lg text-sm font-semibold hover:bg-orange-700">
                    <span wire:loading.remove wire:target="save"><i class="fas fa-check mr-1"></i>Registar Guia</span>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" style="backdrop-filter: blur(2px);">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 text-center">
            ...
            <p class="text-gray-600 text-sm mb-6">A guia será marcada como anulada e removida da lista.</p>
            <div class="flex justify-center gap-2">
                <button wire:click="closeModal" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold">Cancelar</button>
                <button wire:click="deleteGuide" class="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700">Anular</button>
            </div>
        </div>
    </div>
    @endif
