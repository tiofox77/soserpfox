...
<div class="mt-2 flex flex-wrap items-center gap-2">
    <label class="text-xs font-semibold text-gray-600 whitespace-nowrap">
        <i class="fas fa-map-marker-alt mr-1 text-{{ $__cor }}-600"></i>Região fiscal
    </label>
    ...
    <select wire:model.live="tax_country_region"
        ...
        <option value="" @selected(($tax_country_region ?? '') === '')>Automática (província do cliente)</option>
        <option value="AO" @selected(($tax_country_region ?? '') === 'AO')>AO — Angola continental</option>
        <option value="AO-CAB" @selected(($tax_country_region ?? '') === 'AO-CAB')>AO-CAB — Cabinda (regime próprio)</option>
    </select>
