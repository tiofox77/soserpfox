<div class="p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-3xl font-bold text-gray-800 flex items-center">
                    ...
                    {{ __('Proformas de Venda') }}
                </h2>
                <p class="text-gray-600 mt-1">{{ __('Orçamentos e propostas comerciais') }}</p>
            </div>
            <a href="{{ route('invoicing.sales.proformas.create') }}"
               ...
               class="px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-xl font-bold transition-all duration-300 shadow-lg hover:scale-105 active:scale-95">
                <span x-show="!loading"><i class="fas fa-plus mr-2"></i>{{ __('Nova Proforma') }}</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-4 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-200 text-xs font-medium">{{ __('Enviadas') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                <select wire:model.live="statusFilter" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500">
                    ...
                    <option value="sent">{{ __('Enviada') }}</option>
                    <option value="accepted">{{ __('Aceite') }}</option>
                    <option value="rejected">{{ __('Rejeitada') }}</option>
                    <option value="expired">{{ __('Expirada') }}</option>
                    <option value="converted">{{ __('Convertida') }}</option>
                </select>
            </div>
            <div>
                    ...
                    <tr class="hover:bg-purple-50 transition-all duration-200">
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @elseif($proforma->status === 'sent')
                                <span class="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded-full">
                                    <i class="fas fa-paper-plane mr-1"></i>{{ __('Enviada') }}
                                </span>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                <a href="{{ route('invoicing.sales.proformas.preview', $proforma->id) }}" target="_blank"
                                ...
                                <a href="{{ route('invoicing.sales.proformas.edit', $proforma->id) }}"
                                   x-data="{ loading: false }" @click="loading = true"
                                   :class="loading && 'opacity-50 pointer-events-none scale-90'"
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center animate-pulse">
                                ...
                                <p class="text-gray-400 text-sm mt-2">{{ __('Crie a sua primeira proforma de venda') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
