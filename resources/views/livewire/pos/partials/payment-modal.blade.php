    ...
    <div class="bg-white rounded-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto">
        <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-4 flex items-center justify-between rounded-t-2xl">
            <h3 class="text-xl font-bold text-white">
                <i class="fas fa-cash-register mr-2"></i>{{ __('Finalizar Pagamento') }}
            </h3>
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="space-y-2 text-sm">
                    {{-- Os dois pontos vão DENTRO da cadeia: em francês escreve-se
                         "Sous-total :", com espaço antes — e isso é do texto, não
                         do template. --}}
            ...
            <div>
                ...
                <select wire:model="paymentMethod" 
                    ...
                    @if($paymentMethods && $paymentMethods->count() > 0)
                        ...
                        @endforeach
                    @else
                        {{-- Multicaixa, TPA e MB Way ficam como estão: são marcas e
                             siglas, não texto. Traduzi-las era inventar um nome que
            ...
            {{-- Trocos Rápidos --}}
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">⚡ {{ __('Trocos Rápidos') }}</label>
            ...
            {{-- Valor Recebido --}}
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">💰 {{ __('Valor Recebido') }}</label>
            ...
            <div class="bg-gradient-to-br {{ $change >= 0 ? 'from-green-50 to-emerald-50 border-green-300' : 'from-red-50 to-pink-50 border-red-300' }} border-2 rounded-xl p-4">
                <p class="text-sm font-bold {{ $change >= 0 ? 'text-green-800' : 'text-red-800' }} mb-1">
                    {{ $change >= 0 ? '✅ ' . __('TROCO:') : '❌ ' . __('VALOR INSUFICIENTE!') }}
                </p>
            ...
            <div>
                ...
                <textarea wire:model="notes"
                          placeholder="{{ __('Adicione observações...') }}"
                          rows="2"
            ...
            <div class="flex gap-3">
                <button wire:click="$set('showPaymentModal', false)" 
                    ...
                    {{-- "Cancelar" de fechar a janela, não de anular documento:
                         em EN é "Cancel" e não "Cancel document". --}}
