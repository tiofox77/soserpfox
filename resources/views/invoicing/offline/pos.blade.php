@extends('layouts.pwa', ['title' => 'POS Offline'])
...
<div x-data="posOffline()" x-init="init()" x-cloak class="-mx-4 -my-4">
    <div class="lg:grid lg:grid-cols-12 lg:h-[calc(100vh-116px)]">
                ...
                <div class="flex gap-2 items-center">
                    <div class="flex-1 flex items-center gap-2 bg-white rounded-2xl shadow-sm px-3 h-12">
                        ...
                        <input x-model="search" type="search" inputmode="search"
                               placeholder="Pesquisar ou scan código de barras…"
                        ...
                        <button @click="startBarcodeScanner()" class="text-gray-400 hover:text-blue-600 text-lg px-1" title="Ler código de barras com câmara">
                            <i class="fas fa-camera"></i>
                        </button>
                    </div>
                    {{-- Armazém ativo (default do tenant) --}}
                    <div x-show="warehouse" class="shrink-0 h-12 px-3 bg-indigo-100 text-indigo-700 rounded-2xl text-[10px] font-bold flex flex-col items-center justify-center" :title="'Armazém: ' + (warehouse?.name || '')" x-cloak>
                    ...
                    <button @click="manageShift()"
                            ...
                            class="shrink-0 h-12 px-3 rounded-2xl text-[10px] font-bold flex flex-col items-center justify-center transition" title="Gerir turno">
                        <i :class="shift.open ? 'fas fa-lock-open' : 'fas fa-triangle-exclamation'" class="text-sm"></i>
                        <span x-text="shift.open ? 'Turno' : 'S/ turno'"></span>
                    </button>
                    {{-- Pendentes --}}
            ...
            <div class="px-3 pt-3 pb-44 lg:pb-4 lg:flex-1 lg:overflow-y-auto" @scroll.passive="onScroll($event.target)">
                        ...
                        <button @click="addToCart(p)"
                            ...
                            <div class="relative w-full aspect-square rounded-xl bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center mb-2">
                                ...
                                <span x-show="p.type !== 'servico' && p.stock_quantity <= 0"
                                    ...
                                    <span class="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">ESGOTADO</span>
                                </span>
                            </div>
                            ...
                            <div class="mt-auto pt-1.5 flex items-center justify-between gap-1">
                                ...
                                <span x-show="p.type !== 'servico'"
                                      ...
                                      x-text="(p.stock_quantity > 0) ? p.stock_quantity : 'Esgot.'"></span>
                            </div>
                        </button>
                    </template>
                </div>
                ...
                <div x-show="filteredProducts.length > visibleLimit" class="mt-4 text-center">
                    <button @click="loadMore()" class="bg-white border border-gray-200 hover:border-blue-300 text-gray-700 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm transition">
                        <i class="fas fa-chevron-down mr-1"></i>Mostrar mais
                ...
                <p x-show="filteredProducts.length" class="mt-2 text-center text-[11px] text-gray-400">
                    <span x-text="Math.min(visibleLimit, filteredProducts.length)"></span> de <span x-text="filteredProducts.length"></span> produtos
                </p>
                ...
                <div x-show="!filteredProducts.length" class="text-center py-16 text-gray-400">
                    ...
                    <p class="text-sm font-medium" x-text="search || categoryFilter ? 'Nenhum produto encontrado' : 'Sem produtos sincronizados'"></p>
                    <button x-show="!search && !categoryFilter" @click="window.SosPwa.sync(true)" class="mt-3 text-xs bg-blue-600 text-white px-4 py-2 rounded-lg font-bold">
                        <i class="fas fa-rotate mr-1"></i>Sincronizar catálogo
                    </button>
                </div>
            </div>
        </section>
        ...
        <aside class="bg-white flex flex-col
            ...
            <div class="shrink-0 px-4 pt-3 pb-2 border-b border-gray-100">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                        <i class="fas fa-cart-shopping text-emerald-600"></i>Carrinho
                        <span x-show="cart.length" class="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full" x-text="cartCount + (cartCount === 1 ? ' item' : ' itens')"></span>
                    </h2>
                    <div class="flex items-center gap-1">
                        <button @click="clearCart()" x-show="cart.length" class="text-red-500 hover:text-red-600 text-sm px-2 py-1" title="Limpar">
                            <i class="fas fa-trash"></i>
                        </button>
                        <button @click="showCart = false" class="lg:hidden text-gray-400 text-2xl leading-none px-1">&times;</button>
                    </div>
                </div>
                <span class="inline-block mt-1 text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold tracking-wide">FATURA-RECIBO</span>
            </div>
            ...
            <div class="shrink-0 px-4 py-2">
                <button @click="showClientPicker = true"
                    ...
                    <span class="flex-1 truncate" :class="selectedClient ? 'font-semibold text-gray-800' : 'text-gray-500'"
                          x-text="selectedClient ? selectedClient.name : 'Consumidor Final'"></span>
            ...
            <div x-show="!shift.open" x-cloak class="shrink-0 mx-4 mb-2 bg-red-50 border border-red-200 text-red-700 rounded-xl px-3 py-2 text-xs flex items-center gap-2">
                ...
                <span class="flex-1">Sem turno aberto — as vendas podem não entrar no fecho de caixa.</span>
                <button @click="openOpenShiftModal()" class="underline font-bold whitespace-nowrap">Abrir</button>
            </div>
            ...
            <div x-show="shift.open && shift._local" x-cloak class="shrink-0 mx-4 mb-2 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-3 py-2 text-xs flex items-center gap-2">
                ...
                <span class="flex-1">Turno aberto offline — será sincronizado quando a internet voltar.</span>
            </div>
            ...
            <div class="flex-1 overflow-y-auto px-4 space-y-2 min-h-[120px]">
                <template x-for="(itm, idx) in cart" :key="idx">
                    <div class="bg-gray-50 rounded-2xl p-2.5 flex items-center gap-2.5">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[11px] text-gray-500" x-text="formatMoney(itm.unit_price) + ' · ' + (parseFloat(itm.tax_rate) > 0 ? 'IVA ' + itm.tax_rate + '%' : 'Isento')"></p>
                ...
                <div x-show="!cart.length" class="text-center py-10 text-gray-300">
                    ...
                    <p class="text-xs">Toca num produto para adicionar</p>
                </div>
            </div>
            ...
            <template x-if="cartHasOversell">
                <div class="shrink-0 mx-4 mb-1 bg-orange-50 border border-orange-200 text-orange-700 rounded-xl px-3 py-2 text-xs flex items-center gap-2">
                    <i class="fas fa-triangle-exclamation"></i>
                    <span>Atenção: um ou mais itens excedem o stock local disponível. A venda será registada, mas pode ser rejeitada pelo servidor.</span>
                </div>
            </template>
        ...
        {{-- Rodapé: totais + pagamento + ação --}}
            <div class="shrink-0 border-t border-gray-100 px-4 pt-3 pb-4 space-y-3 bg-white">
                ...
                <div class="bg-gradient-to-br from-emerald-600 to-green-700 text-white rounded-2xl shadow-lg p-3">
                    <div class="flex justify-between text-xs opacity-90"><span>Subtotal</span><span x-text="formatMoney(totals.subtotal) + ' Kz'"></span></div>
                    <div x-show="totals.discountAmount > 0" class="flex justify-between text-xs opacity-90 mt-0.5"><span>Desconto (<span x-text="discountPercent"></span>%)</span><span x-text="'-' + formatMoney(totals.discountAmount) + ' Kz'"></span></div>
                    <div x-show="totals.tax > 0" class="flex justify-between text-xs opacity-90 mt-0.5"><span>IVA</span><span x-text="formatMoney(totals.tax) + ' Kz'"></span></div>
                    <div x-show="totals.tax <= 0" class="flex justify-between text-xs opacity-90 mt-0.5"><span>IVA</span><span>Isento</span></div>
                    <div class="border-t border-white/30 mt-1.5 pt-1.5 flex justify-between items-baseline">
                        <span class="font-semibold">TOTAL</span><span class="text-2xl font-extrabold" x-text="formatMoney(totals.total) + ' Kz'"></span>
                    </div>
                </div>
                ...
                <template x-if="cart.length">
                    <div class="space-y-3">
                        {{-- Pagamento --}}
                        <div>
                            <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1.5">Forma de Pagamento</label>
                        ...
                        <div class="flex items-center gap-2">
                            <label class="text-[10px] font-bold text-gray-500 uppercase whitespace-nowrap">Desconto %</label>
                        ...
                        <div x-show="payment === 'cash'" x-cloak class="bg-amber-50 border border-amber-200 rounded-xl p-2.5">
                            <div class="flex items-center gap-2">
                                <label class="text-[11px] font-bold text-amber-800 whitespace-nowrap">Valor recebido</label>
                            ...
                            <div x-show="changeDue > 0" class="flex justify-between mt-2 pt-2 border-t border-amber-200 text-sm font-bold text-amber-900">
                                <span>Troco</span><span x-text="formatMoney(changeDue) + ' Kz'"></span>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <button @click="checkout()" :disabled="!cart.length || saving"
                    ...
                    <span x-show="!saving"><i class="fas fa-circle-check mr-1.5"></i>Finalizar Venda</span>
                    <span x-show="saving"><i class="fas fa-spinner fa-spin mr-1.5"></i>A guardar…</span>
                </button>
    ...
    <div x-show="showClientPicker" @click.self="showClientPicker = false" class="fixed inset-0 bg-black/50 z-[60] flex items-end sm:items-center justify-center" x-transition x-cloak>
        <div class="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl max-h-[80vh] flex flex-col" @click.stop>
            <div class="p-3 border-b flex items-center gap-2">
                ...
                <input x-model="clientSearch" type="search" placeholder="Pesquisar cliente…" class="flex-1 px-1 py-2 text-sm focus:outline-none" autofocus>
            ...
            <div class="overflow-y-auto flex-1 p-2 space-y-1">
                <button @click="selectClient(null); showClientPicker = false" class="w-full text-left p-3 hover:bg-blue-50 rounded-xl flex items-center gap-2">
                    ...
                    <span class="font-semibold text-sm">Consumidor Final</span>
                </button>
                ...
                <div x-show="!filteredClients.length" class="text-center py-6 text-gray-400 text-sm italic">Nenhum cliente</div>
            </div>
    ...
    <div x-show="showClientCreate" @click.self="showClientCreate = false" class="fixed inset-0 bg-black/50 z-[70] flex items-end sm:items-center justify-center" x-transition x-cloak>
        <div class="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl max-h-[88vh] flex flex-col" @click.stop>
            <div class="p-3 border-b flex items-center justify-between">
                <h3 class="font-bold text-base text-gray-900"><i class="fas fa-user-plus mr-1 text-emerald-600"></i>Novo Cliente</h3>
            ...
            <div class="overflow-y-auto flex-1 p-4 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button type="button" @click="newClient.type = 'pessoa_fisica'" :class="newClient.type === 'pessoa_fisica' ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-600 border-gray-200'" class="border-2 rounded-xl py-2.5 text-sm font-bold transition">
                        <i class="fas fa-user mr-1"></i>Singular
                    </button>
                ...
                <div>
                    ...
                    <input x-model="newClient.name" type="text" maxlength="255" placeholder="Nome ou Designação Social"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input x-model="newClient.nif" type="text" maxlength="50" inputmode="numeric" placeholder="Opcional"
                    ...
                    <div>
                        ...
                        <input x-model="newClient.mobile" type="tel" maxlength="50" placeholder="Opcional"
                               class="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-blue-500 focus:outline-none">
                    </div>
                </div>
                <p class="text-[11px] text-gray-400"><i class="fas fa-circle-info mr-1"></i>Guardado localmente e sincronizado automaticamente. Fica logo selecionado nesta venda.</p>
            </div>
            <div class="p-3 border-t flex gap-2">
                <button @click="showClientCreate = false" class="flex-1 py-3 border-2 border-gray-300 text-gray-700 rounded-xl font-bold text-sm">Cancelar</button>
                <button @click="createQuickClient()" :disabled="creatingClient || !newClient.name.trim()" class="flex-[2] bg-gradient-to-r from-emerald-500 to-green-600 text-white py-3 rounded-xl font-bold text-sm shadow-lg disabled:opacity-50">
                    <span x-show="!creatingClient"><i class="fas fa-check mr-1"></i>Guardar e selecionar</span>
                    <span x-show="creatingClient"><i class="fas fa-spinner fa-spin mr-1"></i>A guardar…</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="lastReceipt" @click.self="lastReceipt = null" class="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4" x-transition x-cloak>
        <div class="bg-white rounded-3xl max-w-sm w-full p-6 text-center shadow-2xl" @click.stop>
            ...
            <h3 class="text-xl font-bold text-gray-900 mb-1" x-text="lastReceipt?.synced ? 'Venda Concluída!' : 'Venda Registada (Provisória)'"></h3>
            ...
            <div class="bg-gray-50 rounded-2xl p-3 mb-3 text-left text-sm space-y-0.5">
                <p class="flex justify-between"><span class="text-gray-500">Documento</span><strong x-text="lastReceipt?.number"></strong></p>
                <p class="flex justify-between"><span class="text-gray-500">Cliente</span><strong x-text="lastReceipt?.client"></strong></p>
                <p class="flex justify-between"><span class="text-gray-500">Itens</span><strong x-text="lastReceipt?.itemCount"></strong></p>
                <p class="flex justify-between"><span class="text-gray-500">Pagamento</span><strong x-text="lastReceipt?.payment"></strong></p>
                <p class="flex justify-between text-lg mt-1"><span class="text-gray-500">Total</span><strong class="text-emerald-700" x-text="formatMoney(lastReceipt?.total) + ' Kz'"></strong></p>
            </div>
            ...
            <div class="flex gap-2">
                ...
                <button @click="lastReceipt = null" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-2xl font-bold">Nova Venda</button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showShiftOpen" @click.self="showShiftOpen = false" class="fixed inset-0 bg-black/60 z-[70] flex items-end sm:items-center justify-center" x-transition x-cloak>
        <div class="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl flex flex-col" @click.stop>
            <div class="bg-gradient-to-r from-emerald-600 to-green-700 text-white px-5 py-3 rounded-t-3xl flex items-center justify-between">
                <h3 class="font-bold text-base"><i class="fas fa-lock-open mr-1"></i>Abrir Turno</h3>
            ...
            <div class="p-4 space-y-3">
                <div x-show="!online" x-cloak class="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-3 py-2 text-xs">
                    <i class="fas fa-wifi-slash mr-1"></i>Está offline — o turno abre localmente e sincroniza quando a internet voltar.
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Saldo inicial em caixa (Kz) <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Notas</label>
                    <input x-model="shiftOpenForm.opening_notes" type="text" maxlength="1000" placeholder="Opcional"
                           class="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-emerald-500 focus:outline-none">
                </div>
            </div>
            <div class="p-3 border-t flex gap-2">
                <button @click="showShiftOpen = false" class="flex-1 py-3 border-2 border-gray-300 text-gray-700 rounded-xl font-bold text-sm">Cancelar</button>
                <button @click="confirmOpenShift()" :disabled="shiftBusy || shiftOpenForm.opening_balance === ''"
                    ...
                    <span x-show="!shiftBusy"><i class="fas fa-lock-open mr-1"></i>Abrir turno</span>
                    <span x-show="shiftBusy"><i class="fas fa-spinner fa-spin mr-1"></i>A abrir…</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showShiftClose" @click.self="showShiftClose = false" class="fixed inset-0 bg-black/60 z-[70] flex items-end sm:items-center justify-center" x-transition x-cloak>
        <div class="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl flex flex-col max-h-[90vh]" @click.stop>
            <div class="bg-gradient-to-r from-red-600 to-rose-700 text-white px-5 py-3 rounded-t-3xl flex items-center justify-between">
                <h3 class="font-bold text-base"><i class="fas fa-lock mr-1"></i>Fechar Turno <span class="opacity-80 text-sm" x-text="shift.number ? '· ' + shift.number : ''"></span></h3>
            ...
            <div class="p-4 space-y-3 overflow-y-auto">
                <div x-show="pendingCount > 0" class="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-3 py-2 text-xs">
                    <i class="fas fa-clock mr-1"></i><strong x-text="pendingCount"></strong> documento(s) por sincronizar — o fecho fica em fila e só é efetivado no servidor <strong>depois</strong> de todas as vendas sincronizarem.
                </div>
                <div x-show="!online" x-cloak class="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-3 py-2 text-xs">
                    <i class="fas fa-wifi-slash mr-1"></i>Está offline — o fecho é guardado localmente e sincroniza quando a internet voltar.
                </div>
                <div class="bg-gray-50 rounded-2xl p-3 text-sm space-y-1">
                    <p class="flex justify-between"><span class="text-gray-500">Saldo inicial</span><strong x-text="formatMoney(shift.opening_balance) + ' Kz'"></strong></p>
                    <p class="flex justify-between"><span class="text-gray-500">Vendas dinheiro (sincr.)</span><strong x-text="formatMoney(shift.cash_sales) + ' Kz'"></strong></p>
                    <p class="flex justify-between" x-show="localCashSinceOpen > 0"><span class="text-gray-500">Vendas dinheiro (offline)</span><strong x-text="formatMoney(localCashSinceOpen) + ' Kz'"></strong></p>
                    <p class="flex justify-between border-t pt-1 mt-1"><span class="text-gray-600 font-semibold">Esperado em caixa</span><strong class="text-emerald-700" x-text="formatMoney(expectedCash) + ' Kz'"></strong></p>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Dinheiro contado em caixa (Kz) <span class="text-red-500">*</span></label>
                    ...
                    <p class="text-[11px] mt-1" :class="closeDifference === 0 ? 'text-gray-400' : (closeDifference > 0 ? 'text-emerald-600' : 'text-red-600')"
                       x-show="shiftCloseForm.actual_cash !== ''">
                        Diferença: <strong x-text="(closeDifference > 0 ? '+' : '') + formatMoney(closeDifference) + ' Kz'"></strong>
                    </p>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Notas de fecho</label>
                    <input x-model="shiftCloseForm.closing_notes" type="text" maxlength="1000" placeholder="Opcional"
                ...
                <div x-show="lastSyncDate" class="bg-blue-50 rounded-xl px-3 py-2 text-[11px] text-blue-700 flex items-center gap-1">
                    ...
                    <span>Última sincronização: <strong x-text="lastSyncLabel"></strong></span>
                    <span x-show="pendingCount > 0" class="ml-auto font-bold text-amber-600">Valores locais podem não incluir outros dispositivos.</span>
                </div>
            </div>
            <div class="p-3 border-t flex gap-2">
                <button @click="showShiftClose = false" class="flex-1 py-3 border-2 border-gray-300 text-gray-700 rounded-xl font-bold text-sm">Cancelar</button>
                <button @click="confirmCloseShift()" :disabled="shiftBusy || shiftCloseForm.actual_cash === ''"
                    ...
                    <span x-show="!shiftBusy"><i class="fas fa-lock mr-1"></i>Fechar e Imprimir</span>
                    <span x-show="shiftBusy"><i class="fas fa-spinner fa-spin mr-1"></i>A fechar…</span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showPending" @click.self="showPending = false" class="fixed inset-0 bg-black/60 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4" x-cloak x-transition>
        <div class="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-md max-h-[85vh] flex flex-col" @click.stop>
            <div class="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-5 py-3 rounded-t-3xl flex items-center justify-between">
                <h3 class="font-bold text-base"><i class="fas fa-clock mr-1"></i>Documentos Offline</h3>
            ...
            <div class="p-3 border-b text-xs text-gray-600 flex items-center justify-between gap-2">
                ...
                <button @click="window.SosPwa.sync(true).then(refreshPending)" class="bg-blue-600 text-white px-2.5 py-1 rounded-lg text-[10px] font-bold whitespace-nowrap shrink-0">
                    <i class="fas fa-rotate mr-1"></i>Sincronizar
                </button>
            </div>
            ...
            <template x-if="failedJobs.length">
                <div class="border-b">
                    <div class="px-3 py-2 bg-red-50 flex items-center justify-between">
                        <span class="text-xs font-bold text-red-700"><i class="fas fa-circle-exclamation mr-1"></i><span x-text="failedJobs.length"></span> com erro permanente</span>
                        <button @click="retryAllFailed()" class="text-[10px] bg-red-600 text-white px-2 py-1 rounded-lg font-bold">Tentar todos</button>
                    </div>
                            ...
                            <div class="px-3 py-2 hover:bg-red-50">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[10px] text-red-500 truncate" x-text="j.last_error || 'Erro desconhecido'"></p>
                                    </div>
                                    <button @click="retryJob(j.id)" class="shrink-0 text-[10px] bg-red-600 text-white px-2 py-1 rounded-lg font-bold">Retry</button>
                                </div>
                            </div>
                        </template>
                    </div>
            ...
            <div class="overflow-y-auto flex-1 divide-y">
                    ...
                    <div class="p-3 hover:bg-gray-50">
                        <div class="flex items-start justify-between mb-1">
                            ...
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full ml-2 whitespace-nowrap"
                                  ...
                                  x-text="ps._synced ? '✓ Sincronizada' : '⏳ Pendente'"></span>
                        </div>
                        <div class="flex items-center justify-between">
                ...
                <div x-show="!pendingSales.length && !failedJobs.length" class="p-8 text-center text-gray-400 text-sm italic">Sem documentos registados</div>
            </div>
        </div>
    </div>
</div>
...
function posOffline() {
    return {
        ...
        paymentMethods: [
            { code: 'cash', label: 'Dinheiro', icon: 'fas fa-money-bill-wave' },
            { code: 'card', label: 'Cartão', icon: 'fas fa-credit-card' },
            { code: 'transfer', label: 'Transf.', icon: 'fas fa-university' },
            { code: 'mobile', label: 'Multic.', icon: 'fas fa-mobile-screen' },
        ],
            ...
            window.addEventListener('pwa:pos-sale-synced', async (e) => {
                ...
                if (this.lastSaleRecord && this.lastSaleRecord.local_uuid === e.detail.local_uuid) {
                    ...
                    if (this.lastReceipt) {
                        ...
                        this.lastReceipt.message = 'Sincronizada com a AGT — ' + e.detail.invoice_number;
                    }
                }
            });
        ...
        get lastSyncLabel() {
            if (!this.lastSyncDate) return 'Nunca sincronizado';
            const d = new Date(this.lastSyncDate);
            return 'Sync: ' + d.toLocaleDateString('pt-AO') + ' ' +
        ...
        async confirmOpenShift() {
            ...
            if (isNaN(bal) || bal < 0) { alert('Informe o saldo inicial (0 ou mais).'); return; }
            this.shiftBusy = true;
            try {
                ...
                console.error(err);
                alert('Erro ao abrir turno: ' + err.message);
            } finally {
        ...
        async confirmCloseShift() {
            ...
            if (isNaN(cash) || cash < 0) { alert('Informe o dinheiro contado em caixa (0 ou mais).'); return; }
            this.shiftBusy = true;
            ...
            try {
                ...
                if (!navigator.onLine || this.pendingCount > 0) {
                    alert('Fecho registado.\nSerá efetivado no servidor assim que todas as vendas sincronizarem.');
                }
            } catch (err) {
                console.error(err);
                alert('Erro ao fechar turno: ' + err.message);
            } finally {
                this.shiftBusy = false;
            }
        },
        ...
        async checkout() {
            ...
            if (!this.shift.open && !confirm('⚠️ Não há turno aberto.\nA venda pode não entrar no fecho de caixa. Continuar mesmo assim?')) {
                return;
            }
            this.saving = true;
            try {
                const payLabel = this.paymentMethods.find(m => m.code === this.payment)?.label || 'Dinheiro';
                ...
                const sale = await window.SosPwa.createPosSaleOffline({
                    ...
                    notes: 'POS Offline · Pagamento: ' + payLabel,
                ...
                this.lastReceipt = {
                    ...
                    client: this.selectedClient?.name || 'Consumidor Final',
                    itemCount: this.cartCount,
                    payment: payLabel,
                    total: this.totals.total,
                    message: navigator.onLine
                        ? 'A sincronizar com o servidor…'
                        : 'Guardada localmente — será sincronizada quando voltar online.',
                };
        ...
        async startBarcodeScanner() {
            ...
            if (!('BarcodeDetector' in window)) {
                alert('Leitor de código de barras por câmara não suportado neste dispositivo/navegador.\n\nUse Chrome no Android para esta funcionalidade.');
                return;
            }
            ...
            try {
                ...
                overlay.innerHTML = '<p style="color:#fff;font-size:14px;font-weight:700;margin-bottom:12px"><i class="fas fa-barcode" style="margin-right:6px"></i>Aponte para o código de barras</p>';
