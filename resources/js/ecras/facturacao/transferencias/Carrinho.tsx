                        ...
                        {sugestoes.map((a) => (
                            <li key={a.id}>
                                <button type="button" onClick={() => juntar(a)} className={cls('flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-slate-50', FOCO)}>
                                    ...
                                    <span className={cls('text-xs', a.disponivel > 0 ? 'text-slate-500' : 'text-red-500')}>{t('tem :quanto', { quanto: a.disponivel.toLocaleString('pt-PT') })}</span>
                                </button>
                            </li>
                        ))}
                    </ul>
            ...
            <div className={cls('border border-slate-200', RAIO)}>
                {itens.length === 0 ? <p className="px-3 py-4 text-center text-sm text-slate-400">{t('Sem artigos. Procure e junte.')}</p> : (
                    <table className="w-full text-sm">
                        <thead><tr className="border-b border-slate-200 text-left text-xs uppercase tracking-wider text-slate-500"><th className="px-3 py-2">{t('Artigo')}</th><th className="px-3 py-2 text-right">{t('Tem')}</th><th className="px-3 py-2 text-right">{t('Qtd.')}</th><th className="w-10 px-3 py-2"></th></tr></thead>
                        <tbody className="divide-y divide-slate-100">
                            {itens.map((i, k) => (
                                <tr key={i.product_id}>
                                    ...
                                    <td className="w-32 px-3 py-2"><input type="number" min="0.01" step="0.01" value={i.quantity} onChange={(e) => mudarQuantidade(k, e.target.value)} aria-label={t('Quantidade de :artigo', { artigo: i.product_name })} className={cls(entrada, 'h-8 py-0 text-right tabular-nums')} /></td>
                                    <td className="px-3 py-2 text-right"><button type="button" onClick={() => aoMudar(itens.filter((_, j) => j !== k))} aria-label={t('Tirar :artigo', { artigo: i.product_name })} className={cls('p-1 text-red-500 hover:bg-red-50', RAIO, FOCO)}><i className="fas fa-trash" aria-hidden="true" /></button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
    ...
    return (
        <div className="space-y-4">
            <div className={cls('border border-emerald-200 bg-emerald-50 p-4 text-center', RAIO)}>
                ...
                {pdf && <a href={pdf} target="_blank" rel="noreferrer" className={cls('mt-3 inline-flex items-center gap-2 bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700', RAIO, FOCO)}><i className="fas fa-file-pdf" aria-hidden="true" />{t('Comprovativo em PDF')}</a>}
            </div>
            <table className="w-full text-sm">
