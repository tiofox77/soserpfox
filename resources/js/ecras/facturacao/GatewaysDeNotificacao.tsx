...
export default function GatewaysDeNotificacao() {
    ...
    if (ecra.isError) return <div role="alert" className={cls('border border-red-200 bg-red-50 p-6 text-red-900', RAIO)}>{t('Não foi possível abrir os gateways. Verifique a ligação.')}</div>;
    ...
    return (
        <div className="space-y-4">
            <Faixa icone="fa-bell" titulo={t('Gateways de Notificação')} subtitulo={t('Email, SMS e WhatsApp da empresa, num único lugar')} />
            ...
            <nav aria-label={t('Áreas dos gateways')} className="flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
                ...
                <a href="/notifications/templates" className="ml-auto inline-flex min-h-10 flex-none items-center gap-2 rounded-xl px-4 text-sm font-semibold text-slate-600 hover:bg-slate-50"><i className="fas fa-file-lines text-amber-500" aria-hidden="true" />{t('Gerir templates')}</a>
            </nav>
...
function Email({ forma, mudar, texto, erros, segredos, podeEditar, testar, testando }: { forma: DefinicoesDeGateways; mudar: Mudar; texto: Texto; erros: Record<string, string[]>; segredos: Record<string, boolean>; podeEditar: boolean; testar: () => void; testando: boolean }) {
    return <Cartao titulo={t('Servidor de email')} subtitulo={t('SMTP usado apenas por esta empresa')} icone="fa-envelope" accoes={<Botao icone="fa-paper-plane" disabled={!podeEditar || !forma.from_email} aTrabalhar={testando} onClick={testar}>{t('Enviar email de teste')}</Botao>}><Interruptor etiqueta={t('Activar notificações por email')} valor={forma.email_enabled} aoMudar={(v) => mudar('email_enabled', v)} /><div className="mt-5 grid gap-4 md:grid-cols-2"><Campo etiqueta={t('Servidor SMTP')} obrigatorio={forma.email_enabled} erro={erros.smtp_host}><input className={entrada} value={forma.smtp_host ?? ''} onChange={texto('smtp_host')} placeholder="smtp.exemplo.com" /></Campo><Campo etiqueta={t('Porta')} obrigatorio={forma.email_enabled} erro={erros.smtp_port}><input className={entrada} type="number" min="1" max="65535" value={forma.smtp_port ?? 587} onChange={texto('smtp_port')} /></Campo><Campo etiqueta={t('Utilizador')}><input className={entrada} value={forma.smtp_username ?? ''} onChange={texto('smtp_username')} autoComplete="username" /></Campo><Campo etiqueta={t('Senha')} erro={erros.smtp_password} ajuda={t('Nunca é devolvida ao navegador.')}><input className={entrada} type="password" value={forma.smtp_password ?? ''} onChange={texto('smtp_password')} placeholder={segredo(!!segredos.smtp_password)} autoComplete="new-password" /></Campo><Campo etiqueta={t('Segurança')}><select className={entrada} value={forma.smtp_encryption ?? 'tls'} onChange={texto('smtp_encryption')}><option value="tls">TLS</option><option value="ssl">SSL</option></select></Campo><Campo etiqueta={t('Email remetente')} obrigatorio={forma.email_enabled} erro={erros.from_email}><input className={entrada} type="email" value={forma.from_email ?? ''} onChange={texto('from_email')} /></Campo><Campo etiqueta={t('Nome remetente')} className="md:col-span-2"><input className={entrada} value={forma.from_name ?? ''} onChange={texto('from_name')} placeholder="SOS ERP" /></Campo></div><p className="mt-4 text-xs text-amber-700"><i className="fas fa-triangle-exclamation mr-1" aria-hidden="true" />{t('O botão de teste envia uma mensagem real para o email remetente.')}</p></Cartao>;
}
...
function Sms({ forma, mudar, texto, erros, segredos, podeEditar, testar, testando }: { forma: DefinicoesDeGateways; mudar: Mudar; texto: Texto; erros: Record<string, string[]>; segredos: Record<string, boolean>; podeEditar: boolean; testar: () => void; testando: boolean }) {
    ...
    return <Cartao titulo={t('Gateway de SMS')} subtitulo={t('O remetente TelcoSMS é sempre SOSERP')} icone="fa-comment-sms" accoes={<Botao icone="fa-signal" disabled={!podeEditar || !forma.sms_provider} aTrabalhar={testando} onClick={testar}>{t('Consultar ligação/saldo')}</Botao>}><Interruptor etiqueta={t('Activar notificações por SMS')} valor={forma.sms_enabled} aoMudar={(v) => mudar('sms_enabled', v)} /><div className="mt-5 grid gap-4 md:grid-cols-2"><Campo etiqueta={t('Fornecedor')} obrigatorio={forma.sms_enabled} erro={erros.sms_provider}><select className={entrada} value={forma.sms_provider ?? ''} onChange={texto('sms_provider')}><option value="">{t('Seleccione')}</option><option value="telcosms">TelcoSMS Angola</option><option value="d7networks">D7 Networks</option><option value="twilio">Twilio</option><option value="nexmo">Nexmo / Vonage</option><option value="other">{t('Outro')}</option></select></Campo>{telco || forma.sms_provider === 'd7networks' ? <Campo etiqueta={telco ? 'Chave da aplicação (api_key_app)' : 'API Token'} erro={erros.sms_api_token} ajuda={t('Cifrada; deixe vazio para conservar a chave actual.')}><input className={entrada} type="password" value={forma.sms_api_token ?? ''} onChange={texto('sms_api_token')} placeholder={segredo(!!segredos.sms_api_token)} autoComplete="new-password" /></Campo> : <><Campo etiqueta={t('Conta / API Key')}><input className={entrada} value={forma.sms_account_sid ?? ''} onChange={texto('sms_account_sid')} /></Campo><Campo etiqueta={t('Token / segredo')}><input className={entrada} type="password" value={forma.sms_auth_token ?? ''} onChange={texto('sms_auth_token')} placeholder={segredo(!!segredos.sms_auth_token)} autoComplete="new-password" /></Campo></>}<Campo etiqueta={t('Remetente')}><input className={cls(entrada, telco && 'bg-slate-50')} readOnly={telco} value={telco ? 'SOSERP' : (forma.sms_sender_id ?? '')} onChange={texto('sms_sender_id')} maxLength={30} /></Campo><Campo etiqueta={t('Número de origem')}><input className={entrada} value={forma.sms_from_number ?? ''} onChange={texto('sms_from_number')} placeholder="+244..." /></Campo></div>{telco && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><i className="fas fa-circle-info mr-2" aria-hidden="true" />{t('Use a chave PRD ou QAS da aplicação SOSERP aprovada na TelcoSMS. A consulta acima contacta realmente o fornecedor.')}</div>}</Cartao>;
}
...
function WhatsApp({ forma, mudar, texto, erros, segredos, podeEditar, templates, procurar, procurando, testar, testando }: { forma: DefinicoesDeGateways; mudar: Mudar; texto: Texto; erros: Record<string, string[]>; segredos: Record<string, boolean>; podeEditar: boolean; templates: Array<{ sid: string; name: string; status?: string }>; procurar: () => void; procurando: boolean; testar: (x: { telefone: string; sid: string; nome: string }) => void; testando: boolean }) {
    ...
    return <Cartao titulo={t('WhatsApp Business')} subtitulo={t('Credenciais, templates e número oficial')} icone="fa-brands fa-whatsapp" accoes={<Botao icone="fa-arrows-rotate" disabled={!podeEditar || !forma.whatsapp_account_sid} aTrabalhar={procurando} onClick={procurar}>{t('Procurar templates')}</Botao>}><Interruptor etiqueta={t('Activar notificações por WhatsApp')} valor={forma.whatsapp_enabled} aoMudar={(v) => mudar('whatsapp_enabled', v)} /><div className="mt-5 grid gap-4 md:grid-cols-2"><Campo etiqueta={t('Fornecedor')} obrigatorio={forma.whatsapp_enabled} erro={erros.whatsapp_provider}><select className={entrada} value={forma.whatsapp_provider ?? 'twilio'} onChange={texto('whatsapp_provider')}><option value="twilio">Twilio</option><option value="meta">Meta Cloud API</option></select></Campo><Campo etiqueta={t('Account SID')}><input className={entrada} value={forma.whatsapp_account_sid ?? ''} onChange={texto('whatsapp_account_sid')} /></Campo><Campo etiqueta={t('Token')} ajuda={t('Nunca é devolvido ao navegador.')}><input className={entrada} type="password" value={forma.whatsapp_auth_token ?? ''} onChange={texto('whatsapp_auth_token')} placeholder={segredo(!!segredos.whatsapp_auth_token)} autoComplete="new-password" /></Campo><Campo etiqueta={t('Número de origem')} obrigatorio={forma.whatsapp_enabled} erro={erros.whatsapp_from_number}><input className={entrada} value={forma.whatsapp_from_number ?? ''} onChange={texto('whatsapp_from_number')} placeholder="whatsapp:+244..." /></Campo><Campo etiqueta={t('Business Account ID')}><input className={entrada} value={forma.whatsapp_business_account_id ?? ''} onChange={texto('whatsapp_business_account_id')} /></Campo><Interruptor etiqueta={t('Usar ambiente Sandbox')} valor={forma.whatsapp_sandbox} aoMudar={(v) => mudar('whatsapp_sandbox', v)} /></div>{templates.length > 0 && <div className="mt-5 grid gap-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 md:grid-cols-[1fr_1fr_auto]"><Campo etiqueta={t('Telefone de teste')}><input className={entrada} value={telefone} onChange={(e) => porTelefone(e.target.value)} placeholder="+244..." /></Campo><Campo etiqueta={t('Template')}><select className={entrada} value={sid} onChange={(e) => porSid(e.target.value)}><option value="">{t('Seleccione')}</option>{templates.map((x) => <option key={x.sid} value={x.sid}>{x.name}{x.status ? ` · ${x.status}` : ''}</option>)}</select></Campo><div className="flex items-end"><Botao cor="bom" tom="solida" icone="fa-paper-plane" disabled={!telefone || !escolhido} aTrabalhar={testando} onClick={() => escolhido && testar({ telefone, sid: escolhido.sid, nome: escolhido.name })}>{t('Enviar teste')}</Botao></div></div>}<p className="mt-4 text-xs text-amber-700"><i className="fas fa-triangle-exclamation mr-1" aria-hidden="true" />{t('Procurar templates e enviar teste contactam realmente o fornecedor.')}</p></Cartao>;
}
...
function Preferencias({ canal, forma, mudar, ecra }: { canal: Canal; forma: DefinicoesDeGateways; mudar: Mudar; ecra: EcraDeGateways }) {
    ...
    if (ecra.eventos.length === 0) return <SemNada icone="fa-bell-slash" titulo={t('Sem eventos configuráveis')} frase={t('Crie templates para começar.')} />;
    return <Cartao titulo={t('Quando notificar')} subtitulo={t('Active o evento e, se desejar, associe um template')} icone="fa-bolt" semPadding><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">{t('Evento')}</th><th className="px-5 py-3 text-center">{t('Enviar')}</th><th className="px-5 py-3">{t('Template')}</th></tr></thead><tbody className="divide-y divide-slate-100">{ecra.eventos.map((e, i) => <tr key={e.chave} className="entra hover:bg-slate-50" style={cascata(i)}><td className="px-5 py-3"><span className="inline-flex items-center gap-3 font-medium text-slate-800"><span className="grid h-9 w-9 place-items-center rounded-xl bg-indigo-50 text-indigo-600"><i className={cls('fas', e.icone)} aria-hidden="true" /></span>{e.nome}</span></td><td className="px-5 py-3 text-center"><input type="checkbox" className="h-5 w-5 rounded border-slate-300 text-indigo-600" checked={!!notificacoes[e.chave]} onChange={(x) => alterar(e.chave, x.target.checked)} aria-label={`${notificacoes[e.chave] ? t('Desactivar') : t('Activar')} ${e.nome} por ${canal}`} /></td><td className="px-5 py-3"><select className={cls(entrada, 'max-w-sm')} value={String(mapeamento[e.chave] ?? '')} onChange={(x) => escolher(e.chave, x.target.value)}><option value="">{t('Template padrão')}</option>{templates.map((tpl) => <option key={tpl.id} value={tpl.id}>{tpl.name} · {tpl.module}</option>)}</select></td></tr>)}</tbody></table></div></Cartao>;
}
