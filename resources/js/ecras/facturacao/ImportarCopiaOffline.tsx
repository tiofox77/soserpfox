...
export default function ImportarCopiaOffline() {
    ...
    return (
        <div className="space-y-4" data-copia-offline>
            <Cartao titulo={<span className="flex items-center gap-2"><i className="fas fa-file-import text-slate-400" aria-hidden="true" />{t('Importar Cópia Offline')}</span>}>
                ...
                <label className={cls('flex cursor-pointer flex-col items-center justify-center gap-2 border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-600 hover:border-indigo-400', RAIO)}>
                    ...
                    <span>{nome ? <strong>{nome}</strong> : t('Escolha o ficheiro .json exportado do PWA')}</span>
            ...
            {inventario && (
                <Cartao titulo={t('O que está no ficheiro')} accoes={<Botao cor="primaria" tom="solida" icone="fa-file-import" aTrabalhar={importar.isPending} onClick={() => importar.mutate()}>{t('Importar')}</Botao>}>
                    <div className="grid gap-3 sm:grid-cols-5" data-inventario>
                        {([[t('Vendas'), inventario.vendas], [t('Clientes'), inventario.clientes], [t('Rascunhos'), inventario.rascunhos], [t('Comandas'), inventario.comandas], [t('Turnos'), inventario.turnos]] as Array<[string, number]>).map(([r, v]) => (
            ...
            {resultado && (
                <Cartao titulo={t('Importação concluída')}>
