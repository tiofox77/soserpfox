...
function Valor({ v, formato }: { v: unknown; formato?: Formato }) {
    if (formato === 'estado') {
        const e = ESTADOS[String(v)] ?? { rotulo: String(v ?? '—'), cor: 'neutra' as const };
        return <Etiqueta cor={e.cor}>{e.rotulo}</Etiqueta>;
    }
...
export default function Relatorio({ slug }: { slug: string }) {
    ...
    if (q.isError) {
        return (
            <div className={cls('border border-red-200 bg-red-50 p-6', RAIO)} role="alert">
                <h2 className="mb-2 text-lg font-bold text-red-900">Não foi possível abrir o relatório</h2>
    ...
    return (
        <div className={cls('space-y-4', q.isFetching && 'opacity-70')} data-relatorio={e.slug}>
            <Cartao
                ...
                accoes={
                    <span className="flex flex-wrap gap-2">
                        <a href="/invoicing/reports/novo-ecra" className={cls('inline-flex items-center gap-2 border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50', RAIO)}><i className="fas fa-arrow-left" aria-hidden="true" />Todos os relatórios</a>
                        ...
                        <Botao icone="fa-print" onClick={() => window.print()}>Imprimir</Botao>
                    </span>
                }
            >
            ...
            {e.tabelas.map((t) => <TabelaDoMapa key={t.chave + (t.titulo ?? '')} t={t} dados={dados} />)}
        </div>
    );
}
    ...
    if (f.tipo === 'entidade') {
        return (
            <Campo etiqueta={f.rotulo} className="sm:col-span-2">
                {escolhido ? (
                    <span className="flex items-center gap-2"><Etiqueta cor="primaria" icone="fa-user">{escolhido}</Etiqueta><button type="button" onClick={aoLimpar} className="text-xs text-slate-500 underline">trocar</button></span>
                ) : (
                    <span className="relative block">
                        <input value={procura} onChange={(ev) => porProcura(ev.target.value)} placeholder="Nome ou NIF" className={entrada} aria-autocomplete="list" />
                        {sugestoes.length > 0 && (
        ...
        return (
            <Campo etiqueta={f.rotulo}>
                <select value={v} onChange={(ev) => aoMudar(ev.target.value)} className={entrada}>
                    {(f.opcoes ?? []).every((o) => o.valor !== '') && <option value="">Todos</option>}
...
function TabelaDoMapa({ t, dados }: { t: Tabela; dados: Dados }) {
    const linhas = linhasDe(dados, t.chave);
    const alinhar = (c: Coluna) => (c.alinhar === 'direita' ? 'text-right' : c.alinhar === 'centro' ? 'text-center' : 'text-left');

    return (
        <Cartao titulo={t.titulo} semPadding>
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-slate-200 bg-slate-50 text-left text-xs uppercase tracking-wider text-slate-600">
                            {t.numerada && <th className="px-3 py-2 font-semibold">#</th>}
                            {t.colunas.map((c) => <th key={c.chave + c.rotulo} className={cls('px-3 py-2 font-semibold', alinhar(c))}>{c.rotulo}</th>)}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {linhas.length === 0 && <tr><td colSpan={t.colunas.length + (t.numerada ? 1 : 0)} className="px-3 py-8 text-center text-slate-400">{t.vazio ?? 'Nada a mostrar neste período.'}</td></tr>}
                        {linhas.map((l, i) => (
                            <tr key={i}>
                                {t.numerada && <td className="px-3 py-2 text-slate-400">{i + 1}</td>}
                                {t.colunas.map((c) => <td key={c.chave + c.rotulo} className={cls('px-3 py-2', alinhar(c), (c.formato === 'dinheiro' || c.formato === 'inteiro' || c.formato === 'numero' || c.formato === 'percentagem') && 'tabular-nums')}><Valor v={valor(l, c.chave)} formato={c.formato} /></td>)}
                            </tr>
                        ))}
                    </tbody>
                    {t.rodape && linhas.length > 0 && (
                        <tfoot>
                            <tr className="border-t-2 border-slate-300 bg-slate-50 font-bold">
                                {t.numerada && <td className="px-3 py-2"></td>}
                                {t.colunas.map((c, i) => {
                                    const caminho = t.rodape?.[c.chave];
                                    return <td key={c.chave + c.rotulo} className={cls('px-3 py-2 tabular-nums', alinhar(c))}>{caminho ? formatar(valor(dados, caminho), c.formato ?? 'dinheiro') : i === 0 ? 'Total' : ''}</td>;
                                })}
                            </tr>
                        </tfoot>
                    )}
