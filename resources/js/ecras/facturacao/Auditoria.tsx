...
export default function Auditoria() {
    ...
    return (
        <div className="space-y-4">
            ...
            <Cartao
                titulo={<span className="flex items-center gap-2"><i className="fas fa-clipboard-list text-slate-400" aria-hidden="true" />{t('Auditoria')}</span>}
                accoes={<Botao icone="fa-link" aTrabalhar={verificar.isPending} onClick={() => verificar.mutate()}>{t('Verificar a cadeia')}</Botao>}
            >
                {integridade && (
                    <p role="status" className={cls('mb-4 border px-4 py-3 text-sm', RAIO, integridade.ok ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-red-200 bg-red-50 text-red-900')} data-integridade>
                        {integridade.ok ? t('Cadeia íntegra, verificada às :quando.', { quando: integridade.em }) : t(':total problema(s) na cadeia, verificada às :quando.', { total: integridade.total, quando: integridade.em })}
                    </p>
                )}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className={cls('divide-y divide-slate-100', lista.isFetching && 'opacity-60')}>
                            {linhas.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400">{lista.isPending ? t('A carregar…') : t('Nada registado com estes filtros.')}</td></tr>}
                            {linhas.map((r) => (
                                <tr key={r.id}>
