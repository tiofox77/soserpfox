...
/**
 * REGISTAR UMA FACTURA DE COMPRA.
 *
 ...
 * grava pelo `EmissorDeCompras` — o mesmo que o ecrã Livewire chama.
 *
...
export default function EmitirFacturaDeCompra() {
    ...
    const [feito, porFeito] = useState<{ numero: string; abrir: string } | null>(null);
    ...
    /* O armazém por omissão, quando só há um. */
    useEffect(() => {
        const unico = opcoes.data?.armazens.length === 1 ? opcoes.data.armazens[0] : undefined;
        if (unico) porArmazemId(String(unico.id));
    }, [opcoes.data]);
    ...
    const guardar = useMutation({
        ...
        onSuccess: (r) => { porFeito({ numero: r.numero, abrir: r.abrir }); porErros({}); },
    ...
    if (feito) {
        return (
            <div className={cls(CARTAO, 'p-8 text-center')}>
                ...
                <p className="mt-1 text-sm text-slate-500">Registada. O stock e os lotes já deram entrada.</p>
                <div className="mt-6 flex justify-center gap-2">
                    <Botao cor="primaria" tom="solida" icone="fa-list" onClick={() => (window.location.href = feito.abrir)}>Ver compras</Botao>
                    <Botao icone="fa-plus" onClick={() => { porFeito(null); porLinhas([{ ...LINHA_NOVA }]); porFornecedorId(''); porNotas(''); }}>Registar outra</Botao>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
                <Botao onClick={() => (window.location.href = '/invoicing/purchases/invoices')}>Cancelar</Botao>
                <Botao icone="fa-file" aTrabalhar={aGuardar('draft')} onClick={() => guardar.mutate('draft')}>Guardar rascunho</Botao>
                <Botao icone="fa-money-bill" aTrabalhar={aGuardar('paid')} disabled={!o.permissoes.pode_criar} onClick={() => guardar.mutate('paid')}>Registar como paga</Botao>
