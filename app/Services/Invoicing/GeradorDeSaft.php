...
/**
 * O SAFT-AO — a única implementação.
 *
 * Veio inteiro do ecrã Livewire, onde vivia como métodos privados. O XML é
 * uma declaração à AGT e não pode ter duas versões: o ecrã de sempre e o
 * ecrã em React pedem-no aqui, com o mesmo período e as mesmas secções.
 */
class GeradorDeSaft
{
    public const TIPOS = ['all' => 'Todos', 'sales' => 'Vendas', 'purchases' => 'Compras', 'inventory' => 'Inventário'];

    /** As secções que se podem deixar de fora, e o nome que têm no ecrã. */
    public const SECCOES = [
        'includeProducts' => 'Produtos',
        'includeCustomers' => 'Clientes',
        'includeSuppliers' => 'Fornecedores',
        'includeTaxTable' => 'Tabela de impostos',
        'includeCreditNotes' => 'Notas de crédito',
        'includeDebitNotes' => 'Notas de débito',
        'includePayments' => 'Recibos',
        'includeStockMovements' => 'Movimentos de stock',
    ];

    public bool $includeProducts = true;
    public bool $includeCustomers = true;
    public bool $includeSuppliers = true;
    public bool $includeTaxTable = true;
    public bool $includeCreditNotes = true;
    public bool $includeDebitNotes = true;
    public bool $includePayments = true;
    public bool $includeStockMovements = true;

    public function __construct(
        private readonly int $tenantId,
        public readonly string $startDate,
        public readonly string $endDate,
        public readonly string $documentType = 'all',
        array $seccoes = [],
    ) {
        foreach (array_keys(self::SECCOES) as $chave) {
            if (array_key_exists($chave, $seccoes)) {
                $this->{$chave} = (bool) $seccoes[$chave];
            }
        }
    }
    ...
    public static function regras(): array
    {
        return [
            ...
            'documentType' => ['required', 'in:' . implode(',', array_keys(self::TIPOS))],
        ];
    }

    /** O que o período tem, para o ecrã mostrar antes de gerar. */
    public function estatisticas(): array
    {
        ...
        [$facturas, $valor] = match ($this->documentType) {
            'sales' => [$vendas->count(), (float) $vendas->sum('total')],
            'purchases' => [$compras->count(), (float) $compras->sum('total')],
            'inventory' => [0, 0.0],
            default => [$vendas->count() + $compras->count(), (float) $vendas->sum('total') + (float) $compras->sum('total')],
        };

        return [
            'totalInvoices' => $facturas,
            'totalValue' => $valor,
            'totalCreditNotes' => CreditNote::where('tenant_id', $this->tenantId)
                ->whereBetween('issue_date', [$this->startDate, $this->endDate])
                ->whereNotIn('status', ['draft', 'cancelled'])
                ->count(),
            'totalDebitNotes' => DebitNote::where('tenant_id', $this->tenantId)
                ->whereBetween('issue_date', [$this->startDate, $this->endDate])
                ->whereNotIn('status', ['draft', 'cancelled'])
                ->count(),
            ...
            'totalReceipts' => Receipt::where('tenant_id', $this->tenantId)
                ->where('type', 'sale')
                ->whereBetween('payment_date', [$this->startDate, $this->endDate])
                ->where('status', '!=', 'cancelled')
                ->count(),
            'totalMovements' => StockMovement::where('tenant_id', $this->tenantId)
                ->whereBetween('created_at', [$this->startDate . ' 00:00:00', $this->endDate . ' 23:59:59'])
                ->count(),
            'totalCustomers' => Client::where('tenant_id', $this->tenantId)->count(),
            'totalSuppliers' => Supplier::where('tenant_id', $this->tenantId)->count(),
            'totalProducts' => Product::where('tenant_id', $this->tenantId)->count(),
        ];
    }

    /**
     * Gera, guarda uma cópia e deixa rasto. Devolve o XML e o nome do
     * ficheiro; a descarga é de quem chama, porque cada ecrã a faz à sua
     * maneira.
     *
     * @return array{xml: string, ficheiro: string}
     */
    public function gerar(): array
    {
        $xml = $this->xml();
        $ficheiro = 'SAFT_AO_' . $this->tenantId . '_' . date('Y-m-d') . '.xml';

        Storage::disk('local')->put('saft/' . $ficheiro, $xml);
        ...
        app(AuditRecorder::class)->exportou('SAFT-AO', 'xml', null, [
            'de' => $this->startDate,
            'ate' => $this->endDate,
        ...
        return ['xml' => $xml, 'ficheiro' => $ficheiro];
    }
