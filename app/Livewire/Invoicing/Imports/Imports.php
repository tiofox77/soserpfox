...
/*
 * O REGISTO E O PERCURSO VIVEM NO `GestorDeImportacoes`: o CIF é sempre
 * FOB + frete + seguro, a importação nasce em cotação, e os estados são os
 * do catálogo. Este ecrã e o ecrã em React chamam o mesmo.
 */
#[Layout('layouts.app')]
#[Title('Importações')]
class Imports extends Component
{
    ...
    {
        $this->cif_value = GestorDeImportacoes::cif((float) ($this->fob_value ?: 0), (float) ($this->freight_cost ?: 0), (float) ($this->insurance_cost ?: 0));
    }
    ...
    {
        $gestor = app(GestorDeImportacoes::class);
        ...
        try {
            if ($this->isEditing) {
                $gestor->actualizar($this->editingImport, $dados);
    ...
    public function changeStatus($id, $newStatus)
    {
        try {
            ...
            app(GestorDeImportacoes::class)->mudarEstado($import, (string) $newStatus);
